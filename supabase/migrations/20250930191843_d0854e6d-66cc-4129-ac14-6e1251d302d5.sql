-- Create RPC function for assistjur testemunhas with security and filtering
...
BEGIN
  ...
  WHERE org_id = p_org_id
    AND (v_search = '' OR v_search IS NULL OR (
      COALESCE(nome_testemunha, '') ILIKE ('%' || v_search || '%') OR
      COALESCE(cpf, '') ILIKE ('%' || v_search || '%')
    ))
  ...
  SELECT 
    jsonb_agg(
      jsonb_build_object(
        ...
        END,
        'cpf', CASE 
          WHEN v_user_access_level = 'FULL' THEN COALESCE(t.cpf, '')
          ELSE '***'
        END,
        'qtd_depoimentos', COALESCE(t.qtd_depoimentos, 0),
        'foi_testemunha_em_ambos_polos', COALESCE(t.foi_testemunha_em_ambos_polos, false),
        'ja_foi_reclamante', COALESCE(t.ja_foi_reclamante, false),
        'participou_triangulacao', COALESCE(t.participou_triangulacao, false),
        'participou_troca_favor', COALESCE(t.participou_troca_favor, false),
