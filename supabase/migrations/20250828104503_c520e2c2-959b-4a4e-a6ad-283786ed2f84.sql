-- FASE 1: Views de Qualidade e RPCs para Admin Data Explorer

-- View de qualidade para processos
CREATE OR REPLACE VIEW hubjuria.vw_processos_quality AS
SELECT
  p.*,
  -- Análise de qualidade CNJ
  regexp_replace(COALESCE(p.cnj, ''), '\D', '', 'g') as cnj_digits,
  (length(regexp_replace(COALESCE(p.cnj, ''), '\D', '', 'g')) = 20) as cnj_valid,
  ...
  COUNT(*) OVER (
    PARTITION BY p.org_id, regexp_replace(COALESCE(p.cnj, ''), '\D', '', 'g')
  ) as duplicate_count,
  ROW_NUMBER() OVER (
    PARTITION BY p.org_id, regexp_replace(COALESCE(p.cnj, ''), '\D', '', 'g') 
  ...
  CASE 
    WHEN (length(regexp_replace(COALESCE(p.cnj, ''), '\D', '', 'g')) = 20)
         AND (COALESCE(NULLIF(trim(p.reclamante_nome), ''), '') <> '')
         AND (COALESCE(NULLIF(trim(p.reu_nome), ''), '') <> '')
         AND (COUNT(*) OVER (PARTITION BY p.org_id, regexp_replace(COALESCE(p.cnj, ''), '\D', '', 'g')) = 1)
    THEN 100
    WHEN (length(regexp_replace(COALESCE(p.cnj, ''), '\D', '', 'g')) = 20)
         AND (COALESCE(NULLIF(trim(p.reclamante_nome), ''), '') <> '')
         AND (COALESCE(NULLIF(trim(p.reu_nome), ''), '') <> '')
  ...
  CASE 
    ...
    THEN 'ERROR'
    WHEN (COUNT(*) OVER (PARTITION BY p.org_id, regexp_replace(COALESCE(p.cnj, ''), '\D', '', 'g')) > 1)
    THEN 'WARNING'
    ELSE 'OK'
  END as severity
...
BEGIN
  ...
  -- Extrair apenas dígitos do CNJ
  SELECT regexp_replace(COALESCE(cnj, ''), '\D', '', 'g')
  INTO v_cnj_digits
  FROM processos 
