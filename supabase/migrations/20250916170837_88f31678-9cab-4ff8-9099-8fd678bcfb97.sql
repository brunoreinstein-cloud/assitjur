-- CRITICAL SECURITY FIX: Secure Financial Data Access
...
-- 2. Grant access only through secure functions (not direct view access)
GRANT USAGE ON SCHEMA public TO authenticated;

-- 3. Create comprehensive invoice access audit function
...
BEGIN
  ...
  PERFORM log_user_action(
    ...
    'invoices',
    CASE WHEN TG_OP = 'SELECT' THEN NULL ELSE NEW.id::uuid END,
    access_context
  );
...
-- 4. Create trigger for invoice access auditing
DROP TRIGGER IF EXISTS audit_invoice_access_trigger ON invoices;
CREATE TRIGGER audit_invoice_access_trigger
  AFTER SELECT OR INSERT OR UPDATE OR DELETE ON invoices
  FOR EACH STATEMENT
  EXECUTE FUNCTION audit_invoice_access();

-- 5. Create secure invoice summary function (replaces direct view access)
  ...
  SELECT jsonb_build_object(
    ...
    CASE 
      WHEN EXISTS (
        ...
        AND trigger_name = 'audit_invoice_access_trigger'
      ) THEN 'ACTIVE: Invoice access auditing enabled'
      ELSE 'INACTIVE: Invoice access auditing not enabled'
    END,
