...
serve(async (req) => {
  ...
  try {
    ...
    if (!isSuperAdmin) {
      ...
      return new Response(JSON.stringify({ error: 'Apenas super admins podem resetar senhas' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    if (!targetUserId || !reason || reason.trim().length < 10) {
      return new Response(JSON.stringify({ 
        error: 'targetUserId e reason (mínimo 10 caracteres) são obrigatórios' 
      }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    ...
    if (resetError) {
      ...
      return new Response(JSON.stringify({ error: 'Erro ao enviar email de reset' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    console.log(`Password reset sent for user ${targetUserId} by super admin ${currentUser.id}`);
