  ...
  try {
    ...
    const updates = processos.map((processo, index) => {
      ...
      const reclamanteNome =
        processo.reclamante_nome || `Reclamante ${index + 1}`;

      // Create mock witness arrays based on processo data
      const testemunhasAtivo =
        index % 3 === 0
          ? [
      ...
      const hasTriangulacao =
        testemunhasAtivo.length > 0 && testemunhasPassivo.length > 0;
      ...
      return {
        ...
        classificacao_final: hasTriangulacao
          ? "Risco Alto"
          : hasTrocaDireta
            ? "Risco Médio"
            : "Baixo",
    ...
    console.log(
      `Preparing to update ${updates.length} processos with witness data`,
    );
    ...
    for (let i = 0; i < updates.length; i += batchSize) {
      ...
      console.log(
        `Processed batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(updates.length / batchSize)}`,
      );
    }

    console.log(
      `Successfully processed ${processedCount} processos with witness data`,
    );
