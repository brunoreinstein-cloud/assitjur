  ...
  try {
    ...
    if (req.method === "POST") {
      ...
      if (action === "toggle") {
        ...
        if (enabled) {
          ...
          if (!activeKeys || activeKeys.length === 0) {
            throw new Error(
              "Nenhuma chave OpenAI ativa encontrada. Adicione uma chave válida primeiro.",
            );
          }
      ...
      if (
        ...
        (max_output_tokens < 1 || max_output_tokens > 8000)
      ) {
      ...
      const settingsData = existingSettings
        ? {
            ...
            max_output_tokens:
              max_output_tokens ?? existingSettings.max_output_tokens,
            streaming: streaming ?? existingSettings.streaming,
            rate_per_min: rate_per_min ?? existingSettings.rate_per_min,
            budget_month_cents:
              budget_month_cents ?? existingSettings.budget_month_cents,
            updated_by: userId,
            updated_at: new Date().toISOString(),
          }
        : {
