...
serve(async (req) => {
  ...
  try {
    ...
    if (!isSuperAdmin) {
      ...
      return new Response(JSON.stringify({ error: 'Apenas super admins podem transferir usuários' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    const { targetUserId, newOrgId, reason }: TransferUserRequest = await req.json();

    if (!targetUserId || !newOrgId || !reason || reason.trim().length < 10) {
      return new Response(JSON.stringify({ 
        error: 'targetUserId, newOrgId e reason (mínimo 10 caracteres) são obrigatórios' 
      }), {
        status: 400,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    ...
    if (memberError) {
      ...
      return new Response(JSON.stringify({ error: 'Erro ao atualizar membros' }), {
        status: 500,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }
    ...
    console.log(`User ${targetUserId} transferred from org ${oldOrgId} to ${newOrgId} by super admin ${currentUser.id}`);
