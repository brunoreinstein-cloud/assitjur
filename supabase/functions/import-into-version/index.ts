...
serve("import-into-version", async (req) => {
  console.log(
    `📞 import-into-version called with method: ${req.method}, origin: ${req.headers.get("origin")}`,
  );
  ...
  try {
    ...
    console.log("📥 Received import request:", {
      ...
      firstProcessoFields: requestBody.processos?.[0]
        ? Object.keys(requestBody.processos[0])
        : [],
    ...
    if (requestBody.processos && requestBody.processos.length > 0) {
      ...
      console.log("🔍 DETAILED RECORD ANALYSIS:", {
        ...
        hasRequiredFields: {
          ...
          hasReclamante: !!(
            sampleRecord.reclamante_nome ||
            sampleRecord.reclamante ||
            sampleRecord.reclamante_limpo
          ),
          hasReu: !!(
            sampleRecord.reu_nome ||
            sampleRecord.reu ||
            sampleRecord.reclamado
          ),
        },
      });
    }
    ...
    if (validProcessos.length > 0) {
      console.log(
        `📊 Preparing to insert ${validProcessos.length} processos...`,
      );
      ...
      const processosWithVersion = validProcessos.map(
        (p: any, index: number) => {
          ...
          return {
            ...
            reclamante_nome:
              p.reclamante_nome || p.reclamante_limpo || p.reclamante || "",
            ...
            reclamante_cpf_mask:
              p.reclamante_cpf_mask || p.reclamante_cpf || null,
            data_audiencia:
              p.data_audiencia && p.data_audiencia.match(/^\d{4}-\d{2}-\d{2}$/)
                ? p.data_audiencia
                : null,
      ...
      console.log(
        `🚀 Starting optimized import: ${totalBatches} batches of ${batchSize} records each`,
      );
      console.log(
        `📊 Processing ${processosWithVersion.length} total records with retry mechanism`,
      );
      ...
      for (let i = 0; i < processosWithVersion.length; i += batchSize) {
        ...
        console.log(
          `📦 Processing batch ${batchNumber}/${totalBatches} (records ${i + 1}-${Math.min(i + batchSize, processosWithVersion.length)})`,
        );
        ...
        while (!batchSuccess && retryCount <= maxRetries) {
          if (retryCount > 0) {
            console.log(
              `🔄 Retrying batch ${batchNumber} (attempt ${retryCount + 1}/${maxRetries + 1})`,
            );
            totalRetried++;
            // Wait before retry
          ...
          try {
            ...
            for (const record of batch) {
              try {
                ...
                if (batchInserted < 3) {
                  console.log(
                    `🔍 VALIDATION RECORD ${batchInserted + 1}:`,
                    validation,
                  );
                }
                ...
                if (!record.cnj_digits || record.cnj_digits.length !== 20) {
                  console.warn(
                    `⚠️ Skipping invalid CNJ: "${record.cnj_digits}" (length: ${record.cnj_digits?.length || 0})`,
                  );
                  batchErrors++;
                  continue;
                }
                ...
                console.log(
                  `✅ Processing record with CNJ ${record.cnj_digits}: reclamante="${record.reclamante_nome || "NULL"}", reu="${record.reu_nome || "NULL"}"`,
                );

                // Convert empty strings to null for cleaner data
                if (record.reclamante_nome === "")
                  record.reclamante_nome = null;
                ...
                if (existingRecord) {
                  ...
                  if (updateError) {
                    console.error(
                      `❌ Failed to update CNJ ${record.cnj_digits}:`,
                      {
                    ...
                    if (batchInserted % 5 === 0 || batchInserted < 5) {
                      console.log(
                        `📝 Updated record ${batchInserted}: CNJ ${record.cnj_digits}, Reclamante: "${record.reclamante_nome}", Reu: "${record.reu_nome}"`,
                      );
                    }
                  }
                } else {
                  ...
                  if (insertError) {
                    console.error(
                      `❌ Failed to insert CNJ ${record.cnj_digits}:`,
                      {
                    ...
                    if (batchInserted % 5 === 0 || batchInserted < 5) {
                      console.log(
                        `✅ Inserted record ${batchInserted}: CNJ ${record.cnj_digits}, Reclamante: "${record.reclamante_nome}", Reu: "${record.reu_nome}"`,
                      );
                    }
                  }
                }
                ...
                console.error(
                  `❌ Error processing CNJ ${record.cnj_digits}:`,
                  recordError.message,
                );
                batchErrors++;
              }
            ...
            console.log(
              `✅ Batch ${batchNumber} complete: ${batchInserted}/${batch.length} records processed successfully (${batchErrors} errors)`,
            );
          } catch (batchError: any) {
            console.error(
              `❌ Batch ${batchNumber} failed (attempt ${retryCount + 1}):`,
              batchError.message,
            );
            retryCount++;
            ...
            if (retryCount > maxRetries) {
              console.error(
                `🚨 Batch ${batchNumber} failed after ${maxRetries + 1} attempts, skipping batch`,
              );
        ...
        if (batchNumber % 10 === 0 || batchNumber === totalBatches) {
          ...
          console.log(
            `📊 Progress: ${progressPercent}% complete (${totalInserted} imported, ${errors} errors, ${totalRetried} retries)`,
          );
        }
      }
      ...
      console.log(
        `🎉 Import complete: ${totalInserted}/${validProcessos.length} processos processed`,
      );
      console.log(
        `📊 Final stats: ${totalInserted} successful, ${errors} failed, ${totalRetried} batch retries`,
      );
      ...
      if (errors > 0) {
        ...
        if (failureRate > 0.5) {
          console.error(
            `🚨 High failure rate: ${Math.round(failureRate * 100)}% of records failed (after ${totalRetried} retries)`,
          );
        }
      }
      ...
      if (totalInserted === 0 && validProcessos.length > 0) {
        console.error(
          "🚨 CRITICAL: No records were imported despite having valid data",
        );
        return new Response(
          JSON.stringify({
      ...
      if (validProcessos.length > 1000 && successRate < 0.25) {
        console.error(
          `🚨 Large dataset import with low success rate: ${Math.round(successRate * 100)}%`,
        );
        return new Response(
          JSON.stringify({
            error:
              "Import partially failed - success rate too low for large dataset",
    ...
    const summary = {
      ...
      success_rate:
        validProcessos.length > 0
          ? Math.round((imported / validProcessos.length) * 100)
          : 0,
