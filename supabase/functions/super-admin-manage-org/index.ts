...
serve(async (req) => {
  ...
  try {
    ...
    if (!isSuperAdmin) {
      console.error(`Unauthorized org management attempt by user: ${currentUser.id}`);
      return new Response(JSON.stringify({ error: 'Apenas super admins podem gerenciar organizações' }), {
        status: 403,
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
      });
    }

    const { action, orgId, orgData, reason }: OrgManagementRequest = await req.json();
    ...
    switch (action) {
      ...
      case 'update': {
        if (!orgId) {
          return new Response(JSON.stringify({ error: 'orgId é obrigatório para update' }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
        ...
        if (updateError) {
          ...
          return new Response(JSON.stringify({ error: 'Erro ao atualizar organização' }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
      ...
      case 'toggle_status': {
        if (!orgId) {
          return new Response(JSON.stringify({ error: 'orgId é obrigatório para toggle_status' }), {
            status: 400,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
        ...
        if (toggleError) {
          ...
          return new Response(JSON.stringify({ error: 'Erro ao alterar status da organização' }), {
            status: 500,
            headers: { ...corsHeaders, 'Content-Type': 'application/json' },
          });
        }
    ...
    console.log(`Org management action ${action} completed by super admin ${currentUser.id}`);
