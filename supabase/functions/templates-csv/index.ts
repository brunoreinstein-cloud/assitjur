...
function generateValidCNJ(sequential?: number): string {
  ...
  const cnjWithoutCheckDigits =
    sequentialStr + year + justice + tribunal + origin;
...
const canonicalDicionarioFields = [
  {
    ...
    Obrigatorio: "Sim",
    Regra:
      "String com 20 dígitos (preserva formato original). Validação interna remove pontuação.",
    Exemplo: validCNJs[0],
  },
  {
    Aba: "Por Processo",
...
const dicionarioFields: DicionarioField[] = [
  ...
  },
  {
    ...
    Obrigatorio: "Sim",
    Regra:
      "Lista de CNJs onde atuou como testemunha. Aceita formatos: JSON-like, separado por ; ou ,",
...
serve("templates-csv", async (req) => {
  ...
  try {
    ...
    if (!validSheets.includes(sheet)) {
      return new Response(
        JSON.stringify({
          error: `Aba "${sheet}" inválida. Use: ${validSheets.join(", ")}`,
        }),
        {
          status: 400,
          headers: {
    ...
    const csvContent = buildCanonicalCsv(
      sheet as "Por Processo" | "Por Testemunha" | "Dicionario",
    );
    const filename =
      sheet === "Por Processo"
        ? "por_processo.csv"
