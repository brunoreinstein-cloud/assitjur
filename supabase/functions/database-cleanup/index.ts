...
serve("database-cleanup", async (req) => {
  ...
  try {
    ...
    if (preview) {
      const { data: previewData, error } = await serviceSupabase.rpc(
    ...
    for (const operation of operations) {
      try {
        ...
        switch (operation) {
          case "invalid_cnjs":
            result = await serviceSupabase.rpc("rpc_cleanup_invalid_cnjs", {
              p_org_id: orgId,
            });
            break;
          case "empty_fields":
            result = await serviceSupabase.rpc(
              "rpc_cleanup_empty_required_fields",
              { p_org_id: orgId },
            );
            break;
          case "duplicates":
            result = await serviceSupabase.rpc("rpc_cleanup_duplicates", {
              p_org_id: orgId,
            });
            break;
          case "normalize_cnjs":
            result = await serviceSupabase.rpc("rpc_cleanup_normalize_cnjs", {
              p_org_id: orgId,
            });
            break;
          case "hard_delete_old":
            result = await serviceSupabase.rpc("rpc_cleanup_hard_delete_old", {
              p_org_id: orgId,
            });
            break;
          default:
        ...
        const count =
          result.data?.deleted_count || result.data?.updated_count || 0;
