...
function mapColumns(headers: string[]): Record<string, number> {
  ...
  for (let i = 0; i < headers.length; i++) {
    ...
    for (const [canonical, synonyms] of Object.entries(FIELD_SYNONYMS)) {
      if (
        synonyms.some(
          (synonym) => synonym.toLowerCase() === header.toLowerCase(),
        )
      ) {
...
function detectSheets(workbook: any): {
  processos?: any;
  testemunhas?: any;
  error?: string;
} {
  ...
  for (const name of sheetNames) {
    ...
    if (normalized.includes("porprocesso") || normalized.includes("processo")) {
      ...
      normalized.includes("portestemunha") ||
      normalized.includes("testemunha")
    ) {
...
function validateMinimumFields(
  data: any[],
...
function reconcileData(
  processos: ProcessoRow[],
  testemunhas: TestemunhaRow[],
): { stubs: ProcessoRow[]; issues: ValidationIssue[] } {
  const issues: ValidationIssue[] = [];
...
function calculateAnalyticFlags(
  processos: ProcessoRow[],
  testemunhas: TestemunhaRow[],
): {
  ...
  const testemunhasEnhanced = testemunhas.map((t) => {
    ...
    processos.forEach((p) => {
      if (
        p.reclamantes?.some((r) =>
          r.toLowerCase().includes(t.nome_testemunha.toLowerCase()),
        )
      ) {
    ...
    processos.forEach((p) => {
      if (
        p.testemunhas_ativo?.some((ta) =>
          ta.toLowerCase().includes(t.nome_testemunha.toLowerCase()),
        )
      ) {
        enhanced.foi_testemunha_ativo = true;
      }
      if (
        p.testemunhas_passivo?.some((tp) =>
          tp.toLowerCase().includes(t.nome_testemunha.toLowerCase()),
        )
      ) {
    ...
    enhanced.foi_ambos_polos =
      enhanced.foi_testemunha_ativo && enhanced.foi_testemunha_passivo;
  ...
  const processosEnhanced = processos.map((processo) => {
    ...
    const testemunhasComProva = testemunhasEnhanced
      .filter(
        (t) =>
          ...
          t.cnjs_como_testemunha?.includes(processo.cnj),
      )
    ...
    processo.reclamantes?.forEach((reclamante) => {
      const testemunhaCorrespondente = testemunhasEnhanced.find(
        (t) =>
      ...
      if (
        testemunhaCorrespondente &&
        testemunhaCorrespondente.cnjs_como_testemunha.length > 0
      ) {
        enhanced.reclamante_foi_testemunha = true;
        enhanced.qtd_reclamante_testemunha++;
        enhanced.cnjs_reclamante_testemunha =
          testemunhaCorrespondente.cnjs_como_testemunha;
      }
    });
    ...
    todasTestemunhas.forEach((testemunha) => {
      ...
      processos.forEach((outroProcesso) => {
        ...
        const processoTemTestemunhaDoOutro = [
          ...(outroProcesso.testemunhas_ativo || []),
          ...(outroProcesso.testemunhas_passivo || []),
        ].some((t) =>
          processo.reclamantes?.some((r) =>
...
serve("import-assistjur-xlsx", async (req) => {
  ...
  try {
    ...
    // Log inicial
    await supabase.from("assistjur._ingest_logs").insert({
    ...
    const {
      processos: processosSheet,
      testemunhas: testemunhasSheet,
      error: sheetError,
    } = detectSheets(workbook);
    ...
    const processosRaw = XLSX.utils.sheet_to_json(processosSheet, {
      header: 1,
    });
    const testemunhasRaw = XLSX.utils.sheet_to_json(testemunhasSheet, {
      header: 1,
    });
    ...
    // Normalizar dados
    const processos: ProcessoRow[] = processosRaw
      .slice(1)
      .map((row: any[]) => ({
        ...
        testemunhas_passivo: parseList(
          row[processosColumnMap.testemunhas_passivo],
        ),
        todas_testemunhas: parseList(row[processosColumnMap.todas_testemunhas]),
      }))
      .filter((p) => p.cnj); // Remove linhas sem CNJ

    const testemunhas: TestemunhaRow[] = testemunhasRaw
      .slice(1)
      .map((row: any[]) => ({
        nome_testemunha: normalizeName(
          row[testemunhasColumnMap.nome_testemunha],
        ),
        qtd_depoimentos:
          parseInt(row[testemunhasColumnMap.qtd_depoimentos]) || 0,
        cnjs_como_testemunha: parseList(
          row[testemunhasColumnMap.cnjs_como_testemunha],
        ),
      }))
    ...
    // Reconciliação
    const { stubs, issues: reconcileIssues } = reconcileData(
      processos,
      testemunhas,
    );
    ...
    const {
      processosEnhanced,
      testemunhasEnhanced,
      issues: analyticIssues,
    } = calculateAnalyticFlags(allProcessos, testemunhas);
    ...
    if (processosEnhanced.length > 0) {
      const processosForStaging = processosEnhanced.map((p, index) => ({
        ...
        is_valid: !issues.some(
          (issue) =>
            issue.sheet === "Por Processo" &&
            issue.row === index + 2 &&
            issue.severity === "error",
        ),
        validation_errors: issues.filter(
          (issue) => issue.sheet === "Por Processo" && issue.row === index + 2,
        ),
    ...
    if (testemunhasEnhanced.length > 0) {
      const testemunhasForStaging = testemunhasEnhanced.map((t, index) => ({
        ...
        is_valid: !issues.some(
          (issue) =>
            issue.sheet === "Por Testemunha" &&
            issue.row === index + 2 &&
            issue.severity === "error",
        ),
        validation_errors: issues.filter(
          (issue) =>
            issue.sheet === "Por Testemunha" && issue.row === index + 2,
        ),
      }));
    ...
    const report: ValidationReport = {
      ...
      compliance: {
        lgpd_compliant: true,
        warning_message:
          "⚠️ AVISO LEGAL: Esta análise é preliminar. Validação nos autos é obrigatória antes de qualquer decisão judicial ou administrativa.",
      },
    };
