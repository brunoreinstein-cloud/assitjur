...
serve("review-update-dados", async (req) => {
  ...
  try {
    ...
    console.log(
      `📈 Dados carregados: ${processos.length} processos, ${testemunhas.length} testemunhas`,
    );

    // 2. Normalizar listas
    const { processosNormalizados, testemunhasNormalizadas } = normalizeLists(
      processos,
      testemunhas,
    );

    // 3. Reconciliar CNJs (criar stubs se necessário)
    const { stubs, reconciliationWarnings } = reconcileCNJs(
      processosNormalizados,
      testemunhasNormalizadas,
      orgId,
    );
    ...
    const { processosAtualizados, testemunhasAtualizadas, patternStats } =
      recalculateAnalyticalFlags(todosProcessos, testemunhasNormalizadas);

    // 6. Aplicar classificações
    const { processosClassificados, testemunhasClassificadas } =
      applyClassifications(processosAtualizados, testemunhasAtualizadas);

    // 7. Se não é dry-run, persistir dados
    let upsertResults = { processos: 0, testemunhas: 0 };
    if (!dryRun) {
      upsertResults = await persistUpdatedData(
        orgId,
        processosClassificados,
        testemunhasClassificadas,
      );

      // 8. Gerar/atualizar agregados
      await generateAggregates(
        orgId,
        processosClassificados,
        testemunhasClassificadas,
      );
...
async function fetchExistingData(orgId: string) {
  ...
  const [processosResult, testemunhasResult] = await Promise.all([
    supabase.rpc("get_processos_masked", { org_uuid: orgId }),
    supabase.from("pessoas").select("*").eq("org_id", orgId),
  ]);

  if (processosResult.error)
    throw new Error(
      `Erro ao buscar processos: ${processosResult.error.message}`,
    );
  if (testemunhasResult.error)
    throw new Error(
      `Erro ao buscar testemunhas: ${testemunhasResult.error.message}`,
    );
...
function recalculateAnalyticalFlags(processos: any[], testemunhas: any[]) {
  ...
  const processosAtualizados = processos.map((processo) => {
    ...
    if (duploPapel) {
      ...
      updates.qtd_vezes_reclamante_foi_testemunha =
        duploPapel.cnjs_como_testemunha.length;
      updates.cnjs_em_que_reclamante_foi_testemunha =
        duploPapel.cnjs_como_testemunha;
      stats.duplo_papel++;
    }
 ...
 */
async function persistUpdatedData(
  orgId: string,
  processos: any[],
  testemunhas: any[],
) {
  ...
  const processosLimpos = processos.map(
    ({ is_stub, precisa_completar, ...resto }) => resto,
  );

  const [processosResult, testemunhasResult] = await Promise.all([
    supabase.from("processos").upsert(processosLimpos),
    supabase.from("pessoas").upsert(testemunhas),
  ]);

  if (processosResult.error) {
    throw new Error(
      `Erro ao salvar processos: ${processosResult.error.message}`,
    );
  }

  if (testemunhasResult.error) {
    throw new Error(
      `Erro ao salvar testemunhas: ${testemunhasResult.error.message}`,
    );
  }
...
async function generateAggregates(
  ...
  const agregados = {
    ...
    processos_com_triangulacao: processos.filter(
      (p) => p.triangulacao_confirmada,
    ).length,
    processos_com_troca_direta: processos.filter((p) => p.troca_direta).length,
    processos_com_duplo_papel: processos.filter(
      (p) => p.reclamante_foi_testemunha,
    ).length,
    processos_com_prova_emprestada: processos.filter(
      (p) => p.contem_prova_emprestada,
    ).length,
    testemunhas_profissionais: testemunhas
      .filter((t) => t.e_prova_emprestada)
      .map((t) => ({
