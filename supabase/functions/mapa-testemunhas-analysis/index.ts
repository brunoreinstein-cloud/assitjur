  ...
  for (let i = 0; i < testemunhas.length; i++) {
    for (let j = i + 1; j < testemunhas.length; j++) {
      for (let k = j + 1; k < testemunhas.length; k++) {
        const [nomeA, nomeB, nomeC] = [
          testemunhas[i],
          testemunhas[j],
          testemunhas[k],
        ];

        const ciclo = detectTriangulationCycle(
          [nomeA, nomeB, nomeC],
          processos,
          testemunhaIndex,
        );
...
function detectTriangulationCycle(
  ...
  // Find common CNJs
  const cnjsComuns = dadosA.cnjs.filter(
    (cnj) => dadosB.cnjs.includes(cnj) && dadosC.cnjs.includes(cnj),
  );
  ...
  for (const cnj of cnjsComuns) {
    ...
    const padraoTriangular = checkTriangularPattern(processo, [
      nomeA,
      nomeB,
      nomeC,
    ]);
  ...
  const confianca = calculateTriangulacaoConfianca(
    nomes,
    cnjsComuns,
    Array.from(advogados),
  );
  return {
    ciclo: nomes,
    cnjs: cnjsComuns,
...
function detectDuploPapel(
  ...
  for (const processo of processos) {
    ...
    if (dadosTestemunha && dadosTestemunha.cnjs.length > 1) {
      ...
      const cnjsComoTestemunha = dadosTestemunha.cnjs.filter(
        (cnj: string) => cnj !== processo.cnj,
      );

      // Check if testified in passive pole
      const poloPassivo = dadosTestemunha.polos.some(
        (p: any) => p.polo === "passivo" && cnjsComoTestemunha.includes(p.cnj),
      );

      const risco = calculateDuploPapelRisco(
        cnjsComoReclamante.length,
        cnjsComoTestemunha.length,
        poloPassivo,
      );
...
function detectProvaEmprestada(
  processos: any[],
  testemunhaIndex: Map<string, any>,
  ...
  for (const [nome, dados] of testemunhaIndex) {
    if (dados.cnjs.length > 10) {
      // Threshold for professional witness
...
function detectHomonimos(
  ...
  for (const [nome, dados] of testemunhaIndex) {
    if (dados.cnjs.length > 1) {
      ...
      if (score >= 30) {
        // Only include potential homonyms
        homonimos.push({
          nome,
          score,
          fatores: {
            ...
            temporalidade: calculateTemporalitySimilarity(
              dados.cnjs,
              processos,
            ),
...
function calculatePadroesAgregados(
  processos: any[],
  testemunhaIndex: Map<string, any>,
): PadroesAgregados {
  ...
  const advogadosOfensores = calculateAdvogadosOfensores(
    processos,
    testemunhaIndex,
  );
...
function calculateTrocaConfianca(
  dadosA: any,
  dadosB: any,
  cnjsComuns: string[],
): number {
  ...
  const advogadosComuns = dadosA.advogados.filter((adv: string) =>
    dadosB.advogados.includes(adv),
  );
  score += Math.min(advogadosComuns.length * 5, 20);

  // Same comarca increases confidence
  const comarcasComuns = dadosA.comarcas.filter((comarca: string) =>
    dadosB.comarcas.includes(comarca),
  );
...
function calculateTriangulacaoConfianca(
  nomes: string[],
  cnjs: string[],
  advogados: string[],
): number {
...
function calculateDuploPapelRisco(
  cnjsReclamante: number,
  cnjsTestemunha: number,
  poloPassivo: boolean,
): "ALTO" | "MEDIO" | "BAIXO" {
...
function calculateHomonymScore(
  nome: string,
  dados: any,
  nomesComuns: string[],
): number {
...
function calculateComarcaUFSimilarity(
  comarcas: string[],
  ufs: string[],
): number {
  ...
  return (
    ((comarcas.length - uniqueComarcas) / comarcas.length +
      (ufs.length - uniqueUFs) / ufs.length) *
    50
  );
}
...
function calculateTemporalitySimilarity(
  cnjs: string[],
  processos: any[],
): number {
...
function calculateAdvogadosOfensores(
  processos: any[],
  testemunhaIndex: Map<string, any>,
): string[] {
  const advogadoStats = new Map<
    string,
    {
      testemunhas_profissionais: number;
  ...
  return Array.from(advogadoStats.entries())
    .filter(([_, stats]) => {
      const concentracao =
        stats.total_testemunhas > 0
          ? stats.testemunhas_profissionais / stats.total_testemunhas
          : 0;
