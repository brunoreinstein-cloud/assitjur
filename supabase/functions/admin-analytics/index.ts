  ...
  try {
    ...
    switch (type) {
      case "overview":
        ...
        break;
      case "usage":
        analyticsData = await getUsageAnalytics(
          supabase,
          profile.organization_id,
          period,
        );
        break;
      case "risk_patterns":
        analyticsData = await getRiskPatternAnalytics(
          supabase,
          profile.organization_id,
        );
        break;
      case "performance":
        analyticsData = await getPerformanceAnalytics(
          supabase,
          profile.organization_id,
        );
        break;
      case "detailed_report":
        analyticsData = await getDetailedReport(
          supabase,
          profile.organization_id,
          period,
        );
        break;
      case "ai_patterns":
        analyticsData = await getAIPatternAnalysis(
          supabase,
          profile.organization_id,
        );
        break;
      case "export":
        analyticsData = await generateExportReport(
          supabase,
          profile.organization_id,
          period,
          format,
...
async function getOverviewAnalytics(supabase: any, orgId: string) {
  // Get basic counts
  const [processosResult, pessoasResult, conversationsResult, versionsResult] =
    await Promise.all([
      supabase
        .from("processos")
        .select("*", { count: "exact" })
  ...
  const riskDistribution = {
    ...
    medium:
      riskData?.filter((p) => p.score_risco >= 4 && p.score_risco <= 7)
        .length || 0,
...
async function getUsageAnalytics(
  ...
  // Group by date
  const dailyUsage =
    openaiLogs?.reduce((acc: any, log: any) => {
  ...
  const userStats =
    userActivity?.reduce((acc: any, conv: any) => {
  ...
  return {
    ...
    totalStats: {
      ...
      totalTokens:
        openaiLogs?.reduce(
          (sum: number, log: any) => sum + log.tokens_in + log.tokens_out,
          0,
        ) || 0,
      totalCost:
        openaiLogs?.reduce(
          (sum: number, log: any) => sum + log.cost_cents,
          0,
        ) || 0,
      avgResponseTime:
        openaiLogs?.reduce(
          (sum: number, log: any) => sum + log.duration_ms,
          0,
        ) / (openaiLogs?.length || 1) || 0,
    },
  };
}
...
async function getRiskPatternAnalytics(supabase: any, orgId: string) {
  ...
  const comarcaStats = Object.entries(comarcaRisks)
    .map(([comarca, risks]: [string, any]) => ({
      comarca,
      avgRisk:
        risks.reduce((sum: number, r: number) => sum + r, 0) / risks.length,
      count: risks.length,
  ...
  const tribunalStats = Object.entries(tribunalRisks).map(
    ([tribunal, risks]: [string, any]) => ({
      tribunal,
      avgRisk:
        risks.reduce((sum: number, r: number) => sum + r, 0) / risks.length,
      count: risks.length,
  ...
  return {
    ...
    avgRisk:
      processos.reduce((sum: number, p: any) => sum + p.score_risco, 0) /
      processos.length,
    highRiskPercentage:
      (processos.filter((p: any) => p.score_risco >= 8).length /
        processos.length) *
      100,
  };
}
...
async function getPerformanceAnalytics(supabase: any, orgId: string) {
  ...
  return {
    ...
    metrics: {
      successRate,
      avgImportTime:
        importJobs?.reduce((sum: number, job: any) => {
        ...
        (importJobs?.filter((job: any) => job.started_at && job.completed_at)
          .length || 1),
...
async function getAIPatternAnalysis(supabase: any, orgId: string) {
  ...
  const patterns = {
    ...
    triangulacao:
      processos?.filter((p) => p.triangulacao_confirmada).length || 0,
    provaEmprestada: processos?.filter((p) => p.prova_emprestada).length || 0,
    duploPapel:
      processos?.filter((p) => p.reclamante_foi_testemunha).length || 0,
  };
  ...
  const criticalCombinations =
    processos?.filter(
      (p) =>
        [p.troca_direta, p.triangulacao_confirmada, p.prova_emprestada].filter(
          Boolean,
        ).length >= 2,
    ) || [];
  ...
  return {
    ...
    complexityDistribution: {
      ...
      medium: complexityAnalysis.filter(
        (p) => p.complexity >= 3 && p.complexity <= 5,
      ).length,
...
function generateInsights(usage: any, riskPatterns: any, performance: any) {
  ...
  const avgCostPerRequest =
    usage.totalStats.totalCost / (usage.totalStats.totalRequests || 1);
...
function generateRecommendations(overview: any, usage: any, riskPatterns: any) {
  ...
  if (usage.totalStats.avgResponseTime > 3000) {
    recommendations.push({
      ...
      description:
        "Revisar prompts para reduzir tokens e tempo de processamento",
  ...
  if (riskPatterns.highRiskPercentage > 25) {
    recommendations.push({
      ...
      description: `Investigar ${riskPatterns.comarcaStats
        ?.slice(0, 3)
        .map((c) => c.comarca)
        .join(", ")}`,
...
function calculateRiskCorrelation(complexityAnalysis: any[]) {
  ...
  const avgComplexity =
    validData.reduce((sum, p) => sum + p.complexity, 0) / validData.length;
  const avgRisk =
    validData.reduce((sum, p) => sum + p.score_risco, 0) / validData.length;
