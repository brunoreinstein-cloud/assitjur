...
interface Metrics { durations: number[]; errors: number; total: number }
...
export function serve(name: string, handler: (req: Request) => Promise<Response> | Response) {
  return Deno.serve(async (req) => {
    ...
    try {
      ...
      log.info(`${req.method} ${url.pathname} ${res.status} ${duration.toFixed(2)}ms`);
