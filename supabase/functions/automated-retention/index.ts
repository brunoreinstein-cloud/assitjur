...
serve("automated-retention", async (req) => {
  ...
  try {
    ...
    for (const org of organizations || []) {
      ...
      try {
        ...
        const { data: policies, error: policiesError } = await supabase
          ...
          .select(
            "id, table_name, retention_months, auto_cleanup, next_cleanup_at",
          )
          .eq("org_id", org.id)
        ...
        if (policiesError) {
          console.error(
            `Error fetching policies for org ${org.id}:`,
            policiesError,
          );
          continue;
        }
        ...
        console.log(
          `Found ${policies?.length || 0} policies eligible for cleanup in org ${org.name}`,
        );
        ...
        for (const policy of policies || []) {
          console.log(
            `Executing cleanup for table ${policy.table_name} in org ${org.name}`,
          );
          ...
          try {
            ...
            if (cleanupError) {
              console.error(
                `Cleanup failed for policy ${policy.id}:`,
                cleanupError,
              );
              orgResults.push({
                policyId: policy.id,
              ...
              console.log(
                `Cleanup successful for ${policy.table_name}: ${cleanupResult.records_affected} records affected`,
              );
              orgResults.push({
                policyId: policy.id,
                tableName: policy.table_name,
            ...
            console.error(
              `Error executing cleanup for policy ${policy.id}:`,
              error,
            );
            orgResults.push({
              policyId: policy.id,
    ...
    // Log summary
    const totalPolicies = results.reduce(
      (sum, org) => sum + org.policiesProcessed,
      0,
    );
    const successfulCleanups = results.reduce(
      (sum, org) => sum + (org.results?.filter((r) => r.success).length || 0),
      0,
    );
