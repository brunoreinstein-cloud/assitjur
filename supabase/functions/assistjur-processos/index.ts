...
serve("assistjur-processos", async (req) => {
  ...
  try {
    if (req.method === "GET") {
      return json(
        200,
        { ok: true, service: "assistjur-processos", requestId },
    ...
    if (!auth.startsWith("Bearer ")) {
      ...
      return jsonError(
        401,
        "unauthorized",
        { detail: "missing bearer token", requestId },
        { ...ch, "x-request-id": requestId },
      );
    }
    ...
    try {
      ...
      if (userError || !user) {
        console.error(
          JSON.stringify({
            requestId,
            err: userError?.message ?? "invalid user token",
          }),
        );
        return jsonError(
          401,
          "unauthorized",
          { detail: "invalid user token", requestId },
          { ...ch, "x-request-id": requestId },
        );
      }
      ...
      if (profileError || !profile?.organization_id) {
        console.error(
          JSON.stringify({
            requestId,
            err: profileError?.message ?? "profile_not_found",
          }),
        );
        return jsonError(
          401,
          "unauthorized",
          { detail: "user profile or organization not found", requestId },
          { ...ch, "x-request-id": requestId },
        );
      }
      ...
      return jsonError(
        401,
        "unauthorized",
        { detail: "authentication failed", requestId },
        { ...ch, "x-request-id": requestId },
      );
    }

    try {
      const { data: resultRpc, error } = await supabase.rpc(
      ...
      if (error) {
        ...
        if (code === "42501") {
          ...
          status = 500;
          detail =
            "Função do banco de dados não encontrada. Contate o suporte.";
        } else if (
          error.message?.includes("schema") &&
          error.message?.includes("assistjur")
        ) {
          status = 500;
          detail =
            "Schema assistjur não encontrado. Verifique se os dados foram importados.";
        }
      ...
      return json(
        200,
        { data: processosData, count: totalCount, totalPages, page, requestId },
        { ...ch, "x-request-id": requestId },
      );
    } catch (e) {
      console.error(JSON.stringify({ requestId, err: String(e) }));
