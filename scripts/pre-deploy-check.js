...
function logCheck(type, message) {
  ...
  CHECKS[type === 'error' ? 'errors' : type === 'warning' ? 'warnings' : 'passed'].push(message);
}
...
async function checkConsoleLogs() {
  ...
  const files = await glob('src/**/*.{ts,tsx}', {
    ignore: ['**/*.test.ts', '**/*.test.tsx', '**/src/lib/dev-diagnostics.ts', '**/src/lib/debug-mode.ts'],
  });
  ...
  if (totalLogs === 0) {
    ...
    logCheck('warning', `${totalLogs} console.logs encontrados em ${filesWithLogs.length} arquivos`);
...
async function checkEdgeFunctions() {
  ...
  if (existsSync(configFile)) {
    ...
    if (functionMatches) {
      logCheck('passed', `${functionMatches.length} Edge Functions configuradas`);
    }
  } else {
