...
class ProductionValidator {
  ...
  async checkConsoleLogsInSource() {
        ...
        if (
          ...
          lines.forEach((line, index) => {
            if (
              ...
              !line.includes("// logger substitui")
            ) {
              consoleLogs.push({
    ...
    console.log(
      `   ${consoleLogs.length > 0 ? "⚠️" : "✅"} ${consoleLogs.length} console.logs encontrados\n`,
    );
  }
  ...
  async checkHardcodedCredentials() {
    ...
    try {
      ...
      console.log(
        `   ${hasHardcoded ? "❌" : "✅"} Credenciais ${hasHardcoded ? "ENCONTRADAS" : "seguras"}\n`,
      );
    } catch (error) {
  ...
  async checkEnvironmentVariables() {
    ...
    const missingAlternatives = alternatives
      .filter((pair) => !pair.some((v) => !!process.env[v]))
      .map((pair) => pair.join(" | "));
    ...
    console.log(
      `   ${missingVars.length + missingAlternatives.length > 0 ? "⚠️" : "✅"} ${missingVars.length + missingAlternatives.length} variáveis ausentes\n`,
    );
  }
  ...
  async checkBuildOptimizations() {
    ...
    try {
      ...
      console.log(
        `   ✅ ${Object.keys(optimizations).length - missingOptimizations.length}/${Object.keys(optimizations).length} otimizações ativas\n`,
      );
    } catch (error) {
  ...
  async checkSecurityHeaders() {
    ...
    console.log(
      `   ✅ ${securityScore}/${securityFiles.length} módulos de segurança implementados\n`,
    );
  }
  ...
  generateReport() {
    ...
    const score = Math.max(
      0,
      100 - this.issues.length * 20 - this.warnings.length * 5,
    );
