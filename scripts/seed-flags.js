...
if (!tenantId || !environment) {
  console.error(
    "Usage: node scripts/seed-flags.js --tenant=<id> --env=<dev|staging|prod>",
  );
  process.exit(1);
}
...
const rows = FLAGS.filter((f) => f.environments.includes(environment)).map(
  (f) => ({
    id: f.id,
    tenant_id: tenantId,
    flag: f.flag,
...
console.log(
  `Seeded ${rows.length} flags for tenant ${tenantId} in ${environment}`,
);
