...
const argv = await yargs(hideBin(process.argv))
  .option('url', { type: 'string', default: 'https://assistjur.com.br/', demandOption: true })
  .parse();
...
await fs.promises.writeFile(path.join(outDir, 'snapshot.html'), snapshot, 'utf-8');
...
const head = await page.evaluate(() => ({
  ...
  twitter: {
    ...
    title: document.querySelector('meta[name="twitter:title"]')?.getAttribute('content') || null,
    description: document.querySelector('meta[name="twitter:description"]')?.getAttribute('content') || null,
    image: document.querySelector('meta[name="twitter:image"]')?.getAttribute('content') || null,
  },
}));

const headings = await page.$$eval('h1,h2,h3,h4,h5,h6', els => els.map(e => ({ tag: e.tagName, text: e.textContent?.trim() })));
const imagesWithoutAlt = await page.$$eval('img', imgs => imgs.filter(i => !i.getAttribute('alt') || i.getAttribute('alt') === '').map(i => i.getAttribute('src') || ''));
const links = await page.$$eval('a[href]', as => as.map(a => (a as HTMLAnchorElement).href));

const linkStatuses = await Promise.all(links.map(async href => {
  try {
...
const brokenLinks = linkStatuses.filter(l => typeof l.status === 'number' ? l.status >= 400 : true);

const require = createRequire(import.meta.url);
await page.addScriptTag({ path: require.resolve('axe-core') });
const axe = await page.evaluate(async () => await (window as any).axe.run());
await fs.promises.writeFile(path.join(outDir, 'axe.json'), JSON.stringify(axe, null, 2));
...
async function runLighthouse(kind: 'mobile' | 'desktop') {
  ...
  await fs.promises.writeFile(path.join(lhDir, `${kind}.json`), JSON.stringify(lhr, null, 2));
  return {
    ...
    inp: lhr.audits['interaction-to-next-paint']?.numericValue ?? lhr.audits['experimental-interaction-to-next-paint']?.numericValue,
  };
}
...
if (!sitemapFound) {
  ...
  await fs.promises.writeFile(path.join(fixesDir, 'sitemap.xml'), sitemapContent);
}

const headFixParts: string[] = [];
if (!head.viewport) headFixParts.push('<meta name="viewport" content="width=device-width, initial-scale=1" />');
if (!head.canonical) headFixParts.push(`<link rel="canonical" href="${url}" />`);
if (!head.description) headFixParts.push('<meta name="description" content="" />');
if (!head.og.title) headFixParts.push('<meta property="og:title" content="" />');
if (!head.og.description) headFixParts.push('<meta property="og:description" content="" />');
if (!head.og.image) headFixParts.push('<meta property="og:image" content="https://assistjur.com.br/og-image.jpg" />');
if (!head.og.type) headFixParts.push('<meta property="og:type" content="website" />');
if (!head.og.url) headFixParts.push(`<meta property="og:url" content="${url}" />`);
if (!head.twitter.card) headFixParts.push('<meta name="twitter:card" content="summary_large_image" />');
if (!head.twitter.title) headFixParts.push('<meta name="twitter:title" content="" />');
if (!head.twitter.description) headFixParts.push('<meta name="twitter:description" content="" />');
if (!head.twitter.image) headFixParts.push('<meta name="twitter:image" content="https://assistjur.com.br/og-image.jpg" />');
...
await fs.promises.writeFile(path.join(outDir, 'audit.json'), JSON.stringify(auditData, null, 2));

const topAxe = axe.violations.slice(0, 10).map((v: any) => `- ${v.id}: ${v.help}`).join('\n');
