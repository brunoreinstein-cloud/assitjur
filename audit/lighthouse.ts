...
async function run(kind: 'mobile' | 'desktop') {
  ...
  await fs.promises.writeFile(path.join(outDir, `${kind}.json`), JSON.stringify(lhr, null, 2));
...
console.log('Lighthouse scores (Performance):', { mobile: mobileScore, desktop: desktopScore });
