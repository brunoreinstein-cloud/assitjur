...
describe("feature flags", () => {
  ...
  it("caches flag evaluations and switches users", async () => {
    invoke.mockResolvedValueOnce({
      data: { flags: { "advanced-report": false } },
      error: null,
    });
    ...
    expect(JSON.parse(localStorage.getItem(key1)!).flags).toEqual({
      "advanced-report": false,
    });

    mockUser = { id: "2" } as any;
    mockProfile = { plan: "pro", organization_id: "org2" } as any;
    invoke.mockResolvedValueOnce({
      data: { flags: { "advanced-report": true } },
      error: null,
    });
    rerender(
      <FeatureFlagProvider>
    ...
    expect(JSON.parse(localStorage.getItem(key2)!).flags).toEqual({
