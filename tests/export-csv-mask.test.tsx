...
describe("ExportCsvButton PII masking", () => {
  ...
  it("passes maskPII=true when exporting processos", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: /Exportar dados como CSV/ }),
    );
    fireEvent.click(screen.getByText("Todos os registros"));
    expect(exportProcessosToCSV).toHaveBeenCalledWith(
      expect.any(Array),
      expect.objectContaining({ maskPII: true }),
    );
  });
  ...
  it("passes maskPII=true when exporting testemunhas", () => {
    ...
    fireEvent.click(screen.getByText("Todos os registros"));
    expect(exportTestemunhasToCSV).toHaveBeenCalledWith(
      expect.any(Array),
      expect.objectContaining({ maskPII: true }),
    );
  });
