...
describe('mapa-testemunhas-processos endpoint', () => {
  it('rejects invalid payload', () => {
    const res = processosEndpoint({ filters: 'invalido', limit: 10 })
    expect(res.status).toBe(400)
  })

  it('normalizes missing cursor and limit', () => {
    const res = processosEndpoint({ filters: { temTriangulacao: 'false' } })
    expect(res.status).toBe(200)
    expect(res.body).toMatchObject({ cursor: undefined, limit: 20, filters: { temTriangulacao: false } })
  })

  it('coerces limit from strings', () => {
    const res = processosEndpoint({ limit: '50' })
    expect(res.status).toBe(200)
    expect(res.body).toMatchObject({ limit: 50 })
  })

  it('trims testemunha filter string', () => {
    const res = processosEndpoint({ filters: { testemunha: '  Maria  ' } })
    expect(res.status).toBe(200)
    expect(res.body).toMatchObject({ limit: 20, filters: { testemunha: 'Maria' } })
  })

  it('clamps limit to 200 when above maximum', () => {
    const res = processosEndpoint({ limit: '999' })
    expect(res.status).toBe(200)
    expect(res.body).toMatchObject({ limit: 200, filters: {} })
  })

  it('handles empty payload using defaults', () => {
    const res = processosEndpoint({})
    expect(res.status).toBe(200)
    expect(res.body).toMatchObject({ cursor: undefined, limit: 20, filters: {} })
  })

  it('accepts testemunha filter and trims value', () => {
    const res = processosEndpoint({ filters: { testemunha: '  João  ' } })
    expect(res.status).toBe(200)
    expect(res.body).toMatchObject({ filters: { testemunha: 'João' }, limit: 20 })
  })
})
