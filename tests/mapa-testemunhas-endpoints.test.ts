...
describe('mapa-testemunhas-processos endpoint', () => {
  it('rejects invalid payload', () => {
    const res = processosEndpoint({ filters: 'invalido', limit: 0 })
  ...
  it('normalizes missing cursor and limit', () => {
    ...
    expect(res.body).toMatchObject({ limit: 20, filters: { temTriangulacao: false } })
  })
  ...
  it('handles empty payload using defaults', () => {
    ...
    expect(res.status).toBe(200)
    expect(res.body).toMatchObject({ limit: 20, filters: {} })
  })
