...
export type ClassificacaoEstrategica =
  | "CRÍTICO"
  | "ATENÇÃO"
  | "OBSERVAÇÃO"
  | "NORMAL";
...
export function classificarProcesso(
  ...
  if (factors.triangulacao) {
    ...
    recomendacoes.push(
      "Investigar rede de relacionamentos entre testemunhas e reclamantes",
    );
  }
  ...
  if (factors.duplo_papel) {
    ...
    recomendacoes.push(
      "Investigar histórico da pessoa como reclamante e testemunha",
    );
  }
  ...
  if (factors.advogados_comuns > 2) {
    ...
    recomendacoes.push(
      "Verificar relacionamento entre advogados e testemunhas",
    );
  }
...
function determineClassificacao(
  ...
  const hasCriticoFactor = fatores.some(
    (f) =>
  ...
  const hasAtencaoFactor = fatores.some(
    (f) => f.includes("Duplo papel") || f.includes("ambos os polos"),
  );
...
function generateInsightEstrategico(
  ...
  if (factors.triangulacao) {
    insights.push(
      "Rede circular de testemunhas pode indicar operação coordenada.",
    );
  }

  if (factors.troca_direta) {
    insights.push(
      "Reciprocidade de testemunhas sugere possível acordo entre partes.",
    );
  }
  ...
  if (factors.duplo_papel && factors.ambos_polos) {
    insights.push(
      "Pessoa que alterna entre reclamante e testemunha em diferentes polos é altamente suspeita.",
    );
  }

  if (factors.advogados_comuns > 2) {
    insights.push(
      "Concentração em poucos advogados pode indicar esquema organizado.",
    );
  }

  // Insight final com recomendação
  insights.push(
    "⚖️ Validação nos autos é obrigatória antes de qualquer decisão.",
  );
 ...
 */
export function classificarProcessosBatch(processos: any[]): any[] {
 ...
 */
export function classificarTestemunhasBatch(testemunhas: any[]): any[] {
