...
function buildPessoasPapeisMap(processos: any[]): Map<string, PessoaPapeis> {
  ...
  for (const processo of processos) {
    ...
    for (const testemunha of todasTestemunhas) {
      ...
      const jaAdicionada = pessoasPapeis
        .get(nomeNorm)!
        .testemunha.some((t) => t.cnj === cnj && t.polo !== "indefinido");
...
function buildDuploPapelResult(
  ...
  const timeline = [
    ...
    ...papeis.testemunha.map((t) => ({
      ...
      papel:
        t.polo === "ativo"
          ? ("testemunha_ativo" as const)
          : t.polo === "passivo"
            ? ("testemunha_passivo" as const)
    ...
    if (duploPapel) {
      return {
        ...
        foi_testemunha_em_ambos_polos:
          duploPapel.polo_ativo_testemunha &&
          duploPapel.polo_passivo_testemunha,
      };
    }
