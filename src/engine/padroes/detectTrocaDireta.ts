...
export function updateTrocaDiretaFlags(
  ...
  return processos.map(processo => {
    if (cnjsComTroca.has(processo.cnj)) {
      ...
      const desenho = detalhes.map(d => `${d.testemunhaA} ↔ ${d.testemunhaB}`).join('; ');
      const cnjsRelacionados = [...new Set(detalhes.flatMap(d => [...d.cnjsA, ...d.cnjsB]))];
...
export function updateTestemunhaTrocaFlags(
  ...
  return testemunhas.map(testemunha => {
    ...
    if (envolvimentos && envolvimentos.length > 0) {
      const cnjsRelacionados = [...new Set(envolvimentos.flatMap(e => [...e.cnjsA, ...e.cnjsB]))];
