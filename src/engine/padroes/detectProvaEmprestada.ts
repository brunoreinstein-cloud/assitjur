...
export function detectProvaEmprestada(
  ...
  const testemunhasSuspeitas = testemunhas.filter(t => 
    (t.qtd_depoimentos || 0) > 10
  );
  ...
  return {
    ...
    summary: {
      ...
      media_depoimentos: matches.length > 0 ? Math.round(totalDepoimentos / matches.length) : 0,
...
function buildProvaEmprestadaResult(
  ...
  const processosRelacionados = processos.filter(p => 
    cnjs.includes(p.cnj)
  );
  ...
  const distribuicaoGeografica = analyzeDistribuicaoGeografica(processosRelacionados);
  ...
  const distribuicaoTemporal = analyzeDistribuicaoTemporal(processosRelacionados);
...
function analyzeDistribuicaoGeografica(processos: any[]): {
  ...
  const geografiaCount = new Map<string, { uf: string; comarca: string; count: number }>();
...
function analyzeTimelineSuspeita(distribuicaoTemporal: any[]): boolean {
  ...
  const totalDepoimentos = distribuicaoTemporal.reduce((sum, item) => sum + item.count, 0);
  
  // Verificar se mais de 50% dos depoimentos estão em período ≤ 6 meses
  const sortedByCount = [...distribuicaoTemporal].sort((a, b) => b.count - a.count);
