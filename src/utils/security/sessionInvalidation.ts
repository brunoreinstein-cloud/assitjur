...
export interface SessionInvalidationReason {
  type: 'password_change' | 'security_breach' | 'admin_action' | 'suspicious_activity' | 'device_change';
  message: string;
...
export async function invalidateUserSessions(
  ...
  options: SessionInvalidationOptions = {
    reason: { type: 'security_breach', message: 'Security event', severity: 'high' }
  }
): Promise<{ success: boolean; error?: string }> {
  try {
    const { reason, preserveCurrentSession = false, notifyUser = true } = options;
    ...
    if (!targetUserId) {
      return { success: false, error: 'No user ID provided and no active session' };
    }
    ...
    if (notifyUser && reason.severity !== 'low') {
      const messages = {
        password_change: 'Sua sessão foi encerrada devido à alteração de senha.',
        ...
        suspicious_activity: 'Atividade suspeita detectada. Sua sessão foi encerrada.',
        device_change: 'Acesso de novo dispositivo detectado. Faça login novamente.'
      };

      // This would typically trigger a toast notification
      console.warn('Session invalidated:', messages[reason.type] || reason.message);
    }
...
export class SecurityEventMonitor {
  ...
  async detectSuspiciousActivity(userId: string): Promise<boolean> {
    try {
      ...
      if (failedAttempts > 10) { // More than 10 failures in an hour
      ...
      const actionTimes = recentLogs.map(log => new Date(log.created_at).getTime());
      const rapidActions = actionTimes.filter((time, index) => {
        if (index === 0) return false;
        return time - actionTimes[index - 1] < 1000; // Less than 1 second apart
      }).length;

      if (rapidActions > 20) { // More than 20 rapid actions
      ...
      if (uniqueDevices > 5) { // More than 5 different devices in an hour
