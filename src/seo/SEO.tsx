...
export function SEO({ title, description, path = '/', ogImage, noindex }: SEOProps) {
  useEffect(() => {
    ...
    const image = ogImage?.startsWith('http') ? ogImage : `${BASE_URL}${ogImage ?? DEFAULT_IMAGE}`;
    ...
    updateMeta('meta[name="description"]', { name: 'description', content: finalDesc });
    ...
    updateMeta('meta[property="og:type"]', { property: 'og:type', content: 'website' });
    updateMeta('meta[property="og:url"]', { property: 'og:url', content: canonical });
    updateMeta('meta[property="og:site_name"]', { property: 'og:site_name', content: 'AssistJur' });
    updateMeta('meta[property="og:locale"]', { property: 'og:locale', content: 'pt_BR' });
    updateMeta('meta[property="og:image"]', { property: 'og:image', content: image });
    updateMeta('meta[name="twitter:card"]', { name: 'twitter:card', content: 'summary_large_image' });
    updateMeta('meta[name="twitter:image"]', { name: 'twitter:image', content: image });

    if (finalTitle) {
      updateMeta('meta[property="og:title"]', { property: 'og:title', content: finalTitle });
      updateMeta('meta[name="twitter:title"]', { name: 'twitter:title', content: finalTitle });
    }

    if (finalDesc) {
      updateMeta('meta[property="og:description"]', { property: 'og:description', content: finalDesc });
