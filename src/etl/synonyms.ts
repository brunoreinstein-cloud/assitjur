...
export function resolveFieldName(inputField: string): string | null {
  ...
  for (const [canonical, config] of Object.entries(FIELD_SYNONYMS)) {
    const matchFound = config.synonyms.some(synonym => 
      synonym.toLowerCase().trim() === normalizedInput
    );
 ...
 */
export function mapFieldsUsingSynonyms(inputObject: Record<string, any>): Record<string, any> {
