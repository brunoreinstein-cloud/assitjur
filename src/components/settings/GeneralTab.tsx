...
import { organizationSchema, type Organization } from '@/lib/validation-schemas';
...
export function GeneralTab() {
  ...
  const orgInitials = settings?.name
    ? settings.name.split(' ').slice(0, 2).map(w => w[0]).join('').toUpperCase()
    : 'ORG';
            ...
            <div className="flex flex-col items-center gap-2">
              ...
              <div>
                <p className="text-sm font-medium">
                  {uploadAvatar.isPending ? 'Fazendo upload...' : isDragActive ? 'Solte o logo aqui' : 'Clique ou arraste o logo'}
                </p>
