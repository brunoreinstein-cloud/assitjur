...
export function ClassificationChip({ value, size = 'md' }: ClassificationChipProps) {
  ...
  const getChipStyles = () => {
    // Risco crítico/alto
    if (normalized.includes('crítica') || normalized.includes('alto') || normalized.includes('alta')) {
    ...
    if (normalized.includes('relevante') || normalized.includes('médio') || normalized.includes('media')) {
      return 'bg-yellow-100 text-yellow-800 border-yellow-200 dark:bg-yellow-950/30 dark:text-yellow-400 dark:border-yellow-900';
    }
    
    // Risco baixo/comum
    if (normalized.includes('comum') || normalized.includes('baixo') || normalized.includes('baixa')) {
