...
export function SecurityStatusBanner() {
  ...
  const secureCount = securityItems.filter(
    (item) => item.status === "secure",
  ).length;
  ...
  return (
    <Alert
      className={`border-primary/20 ${warningCount > 0 ? "bg-amber-50" : "bg-primary/5"}`}
    >
      <Shield
        className={`h-4 w-4 ${warningCount > 0 ? "text-amber-600" : ""}`}
      />
      <AlertDescription>
        <div className="flex items-center justify-between">
          <span className="text-sm font-medium">
            Status de Segurança: {secureCount}/{totalCount - warningCount}{" "}
            críticos seguros
            {warningCount > 0 && (
