...
export function CleanupProgress({ isRunning }: CleanupProgressProps) {
  ...
  const steps = [
    ...
    { id: 'cleanup', label: 'Executando limpeza', completed: false, inProgress: isRunning },
      ...
      <div className="space-y-2">
        {steps.map((step) => (
          <div key={step.id} className="flex items-center gap-2 text-sm">
            ...
            <span className={step.completed ? 'text-success' : step.inProgress ? 'text-primary font-medium' : 'text-muted-foreground'}>
              {step.label}
            </span>
          </div>
        ))}
