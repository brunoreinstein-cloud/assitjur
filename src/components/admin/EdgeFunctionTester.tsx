    ...
    try {
      ...
      try {
        const response = await fetch(
          "https://fgjypmlszuzkgvhuszxn.functions.supabase.co/import-into-version",
          {
            method: "OPTIONS",
            headers: {
      ...
      try {
        ...
        const corsMethods = response.headers.get(
      ...
      try {
        const { data: testData, error: testError } =
          await supabase.functions.invoke("import-into-version", {
            body: { test: true, data: [] },
          });
      ...
      try {
        ...
        const { data: createVersionData, error: createVersionError } =
        ...
        newResults.push({
          ...
          message: importError
            ...
            : `Import successful: ${importData?.summary?.imported || 0} records`,
          details: {
            importData,
            importError,
            versionId: createVersionData.versionId,
  ...
  return (
    <Card className="w-full max-w-4xl">
      <CardHeader>
        ...
        <CardDescription>
          Test connectivity and CORS configuration for import-into-version
          function
        </CardDescription>
      </CardHeader>
