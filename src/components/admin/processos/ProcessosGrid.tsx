...
export const ProcessosGrid = memo(function ProcessosGrid({
  ...
  const isAllSelected =
    data.length > 0 && data.every((p) => selectedRows.has(p.id));
  const isPartiallySelected =
    data.some((p) => selectedRows.has(p.id)) && !isAllSelected;
            ...
            {data.map((processo, index) => {
              ...
              return (
                ...
                >
                  <TableCell onClick={(e) => e.stopPropagation()}>
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        onRowSelection(processo.id, !!checked)
                      }
                  ...
                  <TableCell>
                    <TooltipWrapper content={asString(processo.comarca)}>
                      <span className="text-sm">
                        {truncateText(processo.comarca || "—", 15)}
                      </span>
                    </TooltipWrapper>
                  </TableCell>
                  ...
                  <TableCell>
                    <TooltipWrapper
                      content={maskPII(asString(reclamanteText)) ?? ""}
                    >
                      <span className="text-sm">
                        {truncateText(
                          maskPII(asString(reclamanteText)) ?? "",
                          20,
                        )}
                      </span>
                    </TooltipWrapper>
                  ...
                  <TableCell>
                    <TooltipWrapper
                      content={`Atualizado ${formatRelativeTime(processo.updated_at)}`}
                    >
