...
export function ProcessosKPIs({
  ...
  const comTestemunhasAtivas = data.filter(
    (p) => (p.testemunhas_ativo?.length || 0) > 0,
  ).length;

  const semTestemunhas = data.filter(
    (p) =>
      (p.testemunhas_ativo?.length || 0) === 0 &&
      (p.testemunhas_passivo?.length || 0) === 0,
  ).length;

  const aValidar = data.filter(
    (p) =>
