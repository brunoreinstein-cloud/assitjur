...
const STEPS = [
  ...
  {
    id: "validation",
    label: getString("import.steps.validation"),
    icon: CheckCircle2,
  },
  { id: "preview", label: getString("import.steps.preview"), icon: Eye },
  {
    id: "publish",
    label: getString("import.steps.publish"),
    icon: CheckCircle,
  },
] as const;
          ...
          <Card className="border-status-warning/30 bg-status-warning/10">
            ...
            <CardContent className="text-xs text-status-warning space-y-2">
              <p>
                🔒{" "}
                <strong>
                  {getString("import.compliance.maskedData").split(":")[0]}:
                </strong>{" "}
                {getString("import.compliance.maskedData").split(":")[1]}
              </p>
              <p>
                ⚠️{" "}
                <strong>
                  {
                    getString("import.compliance.mandatoryValidation").split(
                      ":",
                    )[0]
                  }
                  :
