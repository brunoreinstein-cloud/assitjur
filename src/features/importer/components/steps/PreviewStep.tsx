...
export function PreviewStep() {
  ...
  const previewData = useMemo(() => {
    ...
    const processos = Array.isArray(validationResult.normalizedData.processos) 
      ...
      : [];
    const testemunhas = Array.isArray(validationResult.normalizedData.testemunhas)
    ...
    return (
      <Card>
        <CardContent className="p-8 text-center">
          ...
          <p className="text-muted-foreground mb-4">Nenhum dado de validação disponível</p>
          <Button 
    ...
    return (
      <Card>
        <CardContent className="p-8 text-center">
          ...
          <p className="text-lg font-medium mb-2">Nenhum dado válido encontrado</p>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {previewData.testemunhas.map((testemunha: any, index: number) => {
                        ...
                        const qtdDepoimentos = typeof testemunha.qtd_depoimentos === 'number'
                          ? testemunha.qtd_depoimentos
                          : parseInt(String(testemunha.qtd_depoimentos || '0'), 10) || 0;
                        
                        // Processamento seguro de CNJs
                        let cnjsInfo = 'N/A';
                        if (testemunha.cnjs_como_testemunha) {
                          if (Array.isArray(testemunha.cnjs_como_testemunha)) {
                        ...
                        return (
                          <TableRow key={index}>
                            <TableCell className="font-medium">
                              {testemunha.nome_testemunha || testemunha.nome_civil || 'N/A'}
                            </TableCell>
                            <TableCell>
