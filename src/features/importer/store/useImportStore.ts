...
export const useImportStore = create<ImportState>((set, get) => ({
  ...
  publishCurrentVersion: async () => {
    ...
    try {
      ...
      const handledError = ErrorHandler.handle(
        error,
        "ImportStore.publishCurrentVersion",
      );
      set({
  ...
  canProceedToNext: () => {
    ...
    switch (currentStep) {
      ...
      case "preview": {
        ...
        const hasProcessos =
          Array.isArray(validationResult.normalizedData.processos) &&
          validationResult.normalizedData.processos.length > 0;
        const hasTestemunhas =
          Array.isArray(validationResult.normalizedData.testemunhas) &&
...
export const selectValidationResult = (state: ImportState) =>
