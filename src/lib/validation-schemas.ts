...
export const personalInfoSchema = z.object({
  ...
  phone: z.string()
    .regex(/^\(\d{2}\) \d{4,5}-\d{4}$/, 'Telefone inválido (formato: (11) 98765-4321)')
    .optional()
    .or(z.literal('')),
...
export const organizationSchema = z.object({
  ...
  cnpj: z.string()
    .regex(/^\d{2}\.\d{3}\.\d{3}\/\d{4}-\d{2}$/, 'CNPJ inválido (formato: 12.345.678/0001-90)')
    .optional()
    .or(z.literal('')),
  primary_color: z.string().regex(/^#[0-9A-Fa-f]{6}$/, 'Cor primária inválida (formato: #RRGGBB)'),
  secondary_color: z.string().regex(/^#[0-9A-Fa-f]{6}$/, 'Cor secundária inválida (formato: #RRGGBB)'),
  session_timeout_minutes: z.number().min(5, 'Mínimo de 5 minutos').max(480, 'Máximo de 8 horas'),
