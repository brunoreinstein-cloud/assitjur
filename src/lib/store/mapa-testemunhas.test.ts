...
describe("mapa-testemunhas store filters", () => {
  ...
  it("removes processo filters when set to undefined", () => {
    ...
    expect(useMapaTestemunhasStore.getState().processoFilters).toEqual({
      status: "ativo",
      uf: "SP",
    });
  ...
  it("removes testemunha filters when set to undefined", () => {
    ...
    expect(useMapaTestemunhasStore.getState().testemunhaFilters).toEqual({
      search: "joao",
      temTroca: true,
    });
