...
export class DevDiagnostics {
  ...
  private setupConsolePatching() {
    ...
    console.error = (...args) => {
      ...
      if (
        stack &&
        !stack.includes("logger.ts") &&
        !stack.includes("error-handling.ts")
      ) {
        logger.warn(
          "Direct console.error usage detected",
          {
            args: args.map((arg) =>
    ...
    console.warn = (...args) => {
      ...
      if (
        ...
        logger.info(
          "Direct console.warn usage detected - consider using logger.warn",
          {
            args: args.map((arg) =>
              typeof arg === "object" ? JSON.stringify(arg) : String(arg),
            ),
  ...
  private setupPerformanceMonitoring() {
    ...
    setInterval(() => {
      if (renderCount > renderThreshold) {
        logger.warn(
          "Excessive component re-renders detected",
          {
            rendersPerSecond: renderCount,
            threshold: renderThreshold,
            suggestion:
              "Check for unnecessary state updates or missing dependencies",
          },
          "DevDiagnostics",
        );
      }
  ...
  private setupHealthChecks() {
    ...
    setInterval(async () => {
      try {
        ...
        const criticalFailures = healthCheck.tests.filter(
          (t) => !t.passed && t.category === "validation",
        );

        if (criticalFailures.length > 0) {
          logger.error(
            "Critical validation tests failing",
            {
  ...
  analyzeBundle() {
    ...
    const performanceTiming = performance.getEntriesByType(
      "navigation",
    ...
    if (performanceTiming) {
      ...
      logger.info(
        "Bundle analysis",
        {
          ...
          suggestion:
            loadTime > 3000 ? "Consider code splitting or lazy loading" : null,
        },
        "DevDiagnostics",
      );
    }
      ...
      if (growthMB > 10) {
        ...
        logger.warn(
          "Potential memory leak detected",
          {
            ...
            growthMB,
            suggestion:
              "Check for uncleaned event listeners or large object references",
          },
          "DevDiagnostics",
        );
      }
  ...
  validateComponentState(
    ...
    if (issues.length > 0) {
      logger.warn(
        "Component state issues detected",
        {
