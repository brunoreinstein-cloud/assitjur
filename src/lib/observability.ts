...
class ObservabilityManager {
  ...
  startTiming(name: string, labels?: Record<string, string>) {
    ...
    return {
        ...
        if (!this.isProduction) {
          logger.debug(`⏱️ ${name} completed`, { duration, labels }, 'Performance');
        }
      }
    };
  }
  ...
  // Error tracking
  recordError(error: Error | string, category: string, labels?: Record<string, string>) {
  ...
  trackApiCall(endpoint: string, method: string, status: number, duration: number) {
    ...
    if (!this.isProduction) {
      logger.info('API call tracked', { endpoint, method, status, duration }, 'API');
    }
  }
  ...
  getMetricsSummary(timeWindow = 5 * 60 * 1000) { // 5 minutos por padrão
    const now = new Date();
  ...
  private getTopErrors(metrics: Metric[]): Array<{error: string, count: number}> {
...
export function useObservability(componentName: string) {
  ...
  const recordInteraction = (action: string, details?: Record<string, string>) => {
