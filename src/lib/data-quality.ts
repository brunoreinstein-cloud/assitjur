...
export function calculateConfidence(
  ...
  const criticalPresent = criticalFields.filter(
    (f) => requiredFields.includes(f) && data[f],
  ).length;
 ...
 */
export function normalizeClassificacao(
...
export function normalizeRiscoNivel(
  ...
  if (insightEstrategico) {
    ...
    if (insight.includes("triangulação") || insight.includes("troca de favor"))
      return "critico";
    if (
      insight.includes("já foi testemunha") ||
      insight.includes("ambos polos")
    )
      return "alto";
