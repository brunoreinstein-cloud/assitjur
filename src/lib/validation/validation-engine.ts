...
export class ValidationEngine {
  ...
  private static readonly COLUMN_SYNONYMS = {
    ...
    'reclamantes': ['reclamantes', 'reclamante', 'autor', 'autores', 'requerente', 'requerentes'],
    'advogados_ativo': [
      'advogados_ativo', 'advogados_parte_ativa', 'advogados (polo ativo)', 
      'advogados polo ativo', 'advogados_autor', 'advogados do autor'
    ],
    'todas_testemunhas': [
      'todas_testemunhas', 'testemunhas_todas', 'testemunhas', 
      'todas as testemunhas', 'lista_testemunhas'
    ],
    'testemunhas_ativo': ['testemunhas_ativo', 'testemunhas_autor', 'testemunhas_parte_ativa'],
    'testemunhas_passivo': ['testemunhas_passivo', 'testemunhas_reu', 'testemunhas_parte_passiva'],
    
    // Testemunha
    'nome_testemunha': ['nome_testemunha', 'nome', 'testemunha', 'nome_completo'],
    'qtd_depoimentos': ['qtd_depoimentos', 'quantidade_depoimentos', 'depoimentos', 'total_depoimentos'],
    'cnjs_como_testemunha': ['cnjs_como_testemunha', 'processos', 'cnjs', 'lista_processos']
  }

  // Nomes comuns que geram alerta de homônimo
  private static readonly COMMON_NAMES = [
    'JOÃO SILVA', 'MARIA SANTOS', 'JOSÉ OLIVEIRA', 'ANA SOUZA', 'CARLOS FERREIRA',
    'FRANCISCA LIMA', 'ANTÔNIO COSTA', 'LUIZA RODRIGUES', 'MANUEL ALVES', 'RITA PEREIRA',
    'FRANCISCO BARBOSA', 'MARIA JOSÉ', 'JOSÉ CARLOS', 'ANA MARIA', 'JOÃO CARLOS'
  ]

  // Campos obrigatórios por sheet
  private static readonly REQUIRED_FIELDS = {
    'Por Processo': ['cnj', 'uf', 'comarca', 'advogados_ativo', 'todas_testemunhas'],
  ...
  static async validateImportData(sheets: ValidationSheet[]): Promise<ValidationResult> {
    ...
    try {
      ...
      const fieldsValidation = this.validateRequiredFields(sheets, columnMappings.mappings)
      errors.push(...fieldsValidation.errors)
      warnings.push(...fieldsValidation.warnings)

      // 4. Processar e normalizar dados
      const processingResult = this.processAndNormalizeData(sheets, columnMappings.mappings)
      ...
      const isValid = errors.filter(e => e.type === 'BLOQUEANTE').length === 0

      return {
        isValid,
        errors,
        warnings,
        summary: this.generateSummary(sheets, processedData, errors, warnings, columnMappings.mappings),
  ...
  private static validateRequiredSheets(sheets: DetectedSheet[]) {
    ...
    const hasProcessos = sheetNames.some(name => 
      name.toLowerCase().includes('processo') || 
      name.toLowerCase() === 'por processo'
    )
    const hasTestemunhas = sheetNames.some(name => 
    ...
    if (!hasProcessos) {
      errors.push({
        ...
        suggestion: 'Certifique-se de que existe uma aba com nome "Por Processo" ou similar',
    ...
    if (!hasTestemunhas) {
      errors.push({
        ...
        suggestion: 'Certifique-se de que existe uma aba com nome "Por Testemunha" ou similar',
  ...
  private static mapColumns(sheets: DetectedSheet[]) {
    ...
    sheets.forEach(sheet => {
      ...
      requiredFields.forEach(requiredField => {
        ...
        } else if (mapping.mapping_type === 'SYNONYM' || mapping.mapping_type === 'FALLBACK') {
          warnings.push({
            type: 'TOLERANCIA',
            severity: 'LOW',
            sheet: sheet.name,
   ...
   */
  private static findColumnMapping(headers: string[], requiredField: string): ColumnMapping {
    const synonyms = this.COLUMN_SYNONYMS[requiredField as keyof typeof this.COLUMN_SYNONYMS] || []
    ...
    for (const synonym of synonyms) {
      for (const header of headers) {
        if (header.toLowerCase().includes(synonym.toLowerCase()) || 
   ...
   */
  private static validateRequiredFields(sheets: DetectedSheet[], mappings: ColumnMapping[]) {
    const errors: ValidationError[] = []
    const warnings: ValidationWarning[] = []
    ...
    sheets.forEach(sheet => {
      ...
      const missingFields = requiredFields.filter(field => 
        !sheetMappings.some(m => m.mapped_name === field && m.mapping_type !== 'NOT_FOUND')
      )
  ...
  private static processAndNormalizeData(sheets: ValidationSheet[], mappings: ColumnMapping[]) {
    ...
    if (processosSheet) {
      const processosResult = this.processProcessosSheet(processosSheet, mappings)
    ...
    if (testemunhasSheet) {
      const testemunhasResult = this.processTestemunhasSheet(testemunhasSheet, mappings)
  ...
  private static processProcessosSheet(sheet: ValidationSheet, mappings: ColumnMapping[]) {
    ...
    sheet.data.forEach((row, index) => {
      ...
      try {
        ...
        const cnj = this.extractMappedValue(row, 'cnj', sheetMappings, sheet.headers)
        const uf = this.extractMappedValue(row, 'uf', sheetMappings, sheet.headers)
        const comarca = this.extractMappedValue(row, 'comarca', sheetMappings, sheet.headers)
        const reclamantes = this.parseListField(
          this.extractMappedValue(row, 'reclamantes', sheetMappings, sheet.headers) || ''
        )
        const advogados_ativo = this.parseListField(
          this.extractMappedValue(row, 'advogados_ativo', sheetMappings, sheet.headers) || ''
        )
        const todas_testemunhas = this.parseListField(
          this.extractMappedValue(row, 'todas_testemunhas', sheetMappings, sheet.headers) || ''
        )
  ...
  private static processTestemunhasSheet(sheet: ValidationSheet, mappings: ColumnMapping[]) {
    ...
    sheet.data.forEach((row, index) => {
      ...
      try {
        const nome_testemunha = this.extractMappedValue(row, 'nome_testemunha', sheetMappings, sheet.headers)
        const qtd_depoimentos = parseInt(
          this.extractMappedValue(row, 'qtd_depoimentos', sheetMappings, sheet.headers) || '0'
        )
        const cnjs_como_testemunha = this.parseListField(
          this.extractMappedValue(row, 'cnjs_como_testemunha', sheetMappings, sheet.headers) || ''
        )

        // Validações específicas
        const nameValid = !!(nome_testemunha && nome_testemunha.trim().length > 0)
        const cnjsValid = cnjs_como_testemunha.every(cnj => this.validateCNJ(cnj))
        const quantityConsistent = qtd_depoimentos === cnjs_como_testemunha.length
  ...
  private static validateIntegrity(data: ProcessedValidationData) {
    ...
    const cnjsFromProcessos = new Set(data.processos.map(p => p.cnj_normalized).filter(Boolean))
    const cnjsFromTestemunhas = new Set(
      data.testemunhas.flatMap(t => t.cnjs_como_testemunha.map(this.normalizeCNJ))
    )
  ...
  private static detectHomonyms(data: ProcessedValidationData) {
    ...
    nameGroups.forEach((variantes, baseName) => {
      if (variantes.length > 1 || this.COMMON_NAMES.some(common => baseName.includes(common))) {
        const score = this.calculateHomonymScore(baseName, variantes, data)
        const probability = score >= 80 ? 'ALTA' : score >= 50 ? 'MEDIA' : 'BAIXA'
        ...
        if (score >= 30) {
          ...
          warnings.push({
            ...
            suggestion: 'Verificar identidade através de CPF/documentos nos autos',
  ...
  private static validateLGPDCompliance(sheets: ValidationSheet[], data?: ProcessedValidationData) {
    ...
    sheets.forEach(sheet => {
      sheet.headers.forEach(header => {
        if (header.toLowerCase().includes('cpf') && !header.toLowerCase().includes('mask')) {
          warnings.push({
            type: 'ATENCAO',
            severity: 'MEDIUM',
            sheet: sheet.name,
            column: header,
            message: 'Coluna de CPF detectada - verificar se não contém dados completos',
  ...
  // Métodos auxiliares
  private static detectSheetType(sheetName: string): keyof typeof ValidationEngine.REQUIRED_FIELDS | null {
    const name = sheetName.toLowerCase().trim()
    if (name.includes('processo') || name === 'por processo') return 'Por Processo'
    if (name.includes('testemunha') || name === 'por testemunha') return 'Por Testemunha'
    return null
  }
  ...
  private static extractMappedValue(row: any[], field: string, mappings: ColumnMapping[], headers: string[]): string | null {
  ...
  private static calculateHomonymScore(baseName: string, variantes: string[], data: ProcessedValidationData): number {
    let score = 0
    ...
    const allCNJs = variantes.flatMap(nome => {
      const testemunha = data.testemunhas.find(t => t.nome_testemunha === nome)
  ...
  private static calculateHomonymFactors(variantes: string[], data: ProcessedValidationData) {
  ...
  private static getCNJsForNames(nomes: string[], data: ProcessedValidationData): string[] {
  ...
  private static generateSummary(
    ...
    return {
      ...
      column_mapping_success: mappings.every(m => m.mapping_type !== 'NOT_FOUND'),
      integrity_check_passed: data?.integrity_links.every(l => l.exists_in_processos || l.needs_stub_creation) ?? false,
