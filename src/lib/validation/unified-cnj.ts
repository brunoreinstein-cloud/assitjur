...
function validateCNJCheckDigits(cnj: string): boolean {
  ...
  const sum1 = sequence1.reduce((acc, digit, idx) => acc + digit * weights1[idx], 0);
  const checkDigit1 = sum1 % 11 < 2 ? 0 : 11 - (sum1 % 11);
  
  // Segundo dígito verificador
  const sequence2 = digits.slice(7, 13);
  const weights2 = [2, 3, 4, 5, 6, 7];
  const sum2 = sequence2.reduce((acc, digit, idx) => acc + digit * weights2[idx], 0);
...
export function validateCNJ(
  ...
  if (mode === 'correction') {
    if (cleaned.length < 15) {
      errors.push(`CNJ muito curto: ${cleaned.length} dígitos (mínimo 15 para correção)`);
      return {
        isValid: false,
        formatted: cleaned,
        cleaned,
  ...
  if (cleaned.length !== 20) {
    errors.push(`CNJ deve ter exatamente 20 dígitos, encontrados: ${cleaned.length}`);
  }
 ...
 */
export function correctCNJ(cnj: any): { corrected: string; needsCorrection: boolean; reason?: string } {
  ...
  return {
    ...
    reason: original !== corrected ? `CNJ completado de ${original.length} para 20 dígitos` : undefined
  };
}