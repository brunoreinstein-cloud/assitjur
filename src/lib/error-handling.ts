...
export const createError = {
  ...
  business: (message: string, userMessage?: string, context?: Record<string, any>) =>
  ...
  system: (message: string, context?: Record<string, any>) =>
    new AssistJurError(message, 'system', {
      userMessage: 'Erro interno do sistema. Tente novamente em alguns minutos.',
      context
    })
};
  ...
  static handle(error: unknown, service?: string): AppError {
    ...
    if (error instanceof AssistJurError) {
      ...
      if (error.message?.includes('fetch')) {
        appError = createError.network(error.message, true, { originalError: error.message });
      } else if (error.message?.includes('auth') || error.message?.includes('session')) {
        appError = createError.authentication(error.message, { originalError: error.message });
      } else {
        appError = createError.system(error.message, { originalError: error.message });
      }
    } else {
      appError = createError.system('Erro desconhecido', { originalError: String(error) });
    }
...
export function isValidOrgId(orgId: string | null | undefined): orgId is string {
