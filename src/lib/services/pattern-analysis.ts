...
export class PatternAnalysisService {
  ...
  private static generateRecommendations(analysis: AnalysisResult): string[] {
    ...
    const highRiskDualRole = analysis.duploPapel.filter(
      (d) => d.risco === "ALTO",
    );
    if (highRiskDualRole.length > 0) {
      recommendations.push(
        `Priorizar contradita em ${highRiskDualRole.length} caso(s) de duplo papel de alto risco`,
      );
    }

    // Professional witnesses
    const professionalWitnesses = analysis.provaEmprestada.filter(
      (p) => p.alerta,
    );
    if (professionalWitnesses.length > 0) {
      recommendations.push(
        `Investigar ${professionalWitnesses.length} testemunha(s) profissional(is) - evitar contradita automática`,
      );
    }

    // High-confidence direct exchanges
    const highConfidenceTroca = analysis.trocaDireta.filter(
      (t) => t.confianca >= 80,
    );
    if (highConfidenceTroca.length > 0) {
      recommendations.push(
        `Explorar ${highConfidenceTroca.length} padrão(ões) de troca direta com alta confiança`,
      );
    }
    ...
    if (analysis.triangulacao.length > 0) {
      recommendations.push(
        `Analisar ${analysis.triangulacao.length} rede(s) de triangulação detectada(s)`,
      );
    }

    // High-probability homonyms
    const highProbHomonyms = analysis.homonimos.filter(
      (h) => h.probabilidade === "ALTA",
    );
    if (highProbHomonyms.length > 0) {
      recommendations.push(
        `Verificar identidade de ${highProbHomonyms.length} possível(is) homônimo(s)`,
      );
    }
    ...
    if (analysis.padroes.advogados_ofensores.length > 0) {
      recommendations.push(
        `Monitorar ${analysis.padroes.advogados_ofensores.length} advogado(s) com padrão ofensivo`,
      );
    }

    // Geographic concentration
    const topUF = Object.entries(analysis.padroes.concentracao_uf).sort(
      ([, a], [, b]) => b - a,
    )[0];
    if (topUF && topUF[1] > analysis.padroes.total_processos * 0.3) {
      recommendations.push(
        `Concentração de casos em ${topUF[0]} (${topUF[1]} processos) - verificar padrão regional`,
      );
    }
