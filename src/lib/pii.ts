...
export const maskCPF = (cpf: string | null | undefined): string => {
  ...
  if (cleaned.length >= 3) {
    return (
      ...
      "*".repeat(Math.max(1, cleaned.length - 4)) +
      cleaned.slice(-2)
    );
  }
...
export const maskCNPJ = (cnpj: string | null | undefined): string => {
  ...
  if (cleaned.length >= 6) {
    return (
      cleaned.substring(0, 2) +
      "*".repeat(Math.max(1, cleaned.length - 6)) +
      cleaned.slice(-4)
    );
  }
...
export const maskEmail = (email: string | null | undefined): string => {
  ...
  const maskedLocal =
    localPart.substring(0, keepChars) +
    "*".repeat(Math.max(1, localPart.length - keepChars));
...
export const maskOAB = (oab: string | null | undefined): string => {
  ...
  if (numbers.length >= 4) {
    ...
    return oab.replace(/\d/g, (digit, index, str) => {
      const numberIndex =
        str.substring(0, index + 1).replace(/\D/g, "").length - 1;
...
export const maskName = (name: string | null | undefined): string => {
  ...
  return words
    .map((word) => {
      ...
      return (
        word.substring(0, 2) +
        "*".repeat(Math.max(0, word.length - 3)) +
        word.slice(-1)
      );
    })
    .join(" ");
...
export const applyPIIMask = (text: string | null | undefined): string => {
  ...
  result = result.replace(/\b\d{3}\.?\d{3}\.?\d{3}-?\d{2}\b/g, (match) =>
    maskCPF(match),
  );
  ...
  // Phone patterns
  result = result.replace(/\(\d{2}\)?\s*\d{4,5}-?\d{4}\b/g, (match) =>
    maskPhone(match),
  );
