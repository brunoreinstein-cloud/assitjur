...
export class RegressionDetector {
  ...
  registerTest(testCase: TestCase) {
    ...
    logger.info('Test case registered', { 
      name: testCase.name, 
    ...
    for (const testCase of this.testCases) {
      ...
      try {
        ...
        if (!passed && testCase.critical) {
          logger.error('Critical test failed', {
            testName: testCase.name,
            category: testCase.category
          }, 'RegressionDetector');
        }
  ...
  private generateTestSummary(results: TestResult[]) {
    ...
    const criticalFailures = results.filter(r => !r.passed && 
      this.testCases.find(t => t.name === r.name)?.critical).length;
...
export class PerformanceMonitor {
  ...
  monitorApiCall<T>(
    ...
    return new Promise((resolve, reject) => {
      const timer = observability.startTiming('api_call_monitored', { endpoint });
  ...
  checkMemoryUsage(): { usage: number; warning: boolean } {
    ...
    if (warning) {
      logger.warn('High memory usage detected', {
        ...
        threshold: Math.round(this.thresholds.memoryUsage / 1024 / 1024) + 'MB'
      }, 'PerformanceMonitor');
    }
...
regressionDetector.registerTest({
  ...
  test: () => {
    try {
      ...
      return handled.category === 'validation' && handled.message === 'Test error';
    } catch {
      return false;
    }
  }
