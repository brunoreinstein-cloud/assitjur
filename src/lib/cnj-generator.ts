...
export function generateValidCNJ(options?: {
  ...
  const sequential = options?.sequential || String(Math.floor(Math.random() * 9999999) + 1).padStart(7, '0');
  const justice = options?.justice || '5'; // Justiça do Trabalho
  const tribunal = options?.tribunal || String(Math.floor(Math.random() * 24) + 1).padStart(2, '0');
  const origin = options?.origin || String(Math.floor(Math.random() * 9999) + 1).padStart(4, '0');
 ...
 */
export function generateSequentialCNJs(count: number, startYear = 2024): string[] {
  const cnjs: string[] = [];
  const baseSequential = 1000000;
  
  for (let i = 0; i < count; i++) {
    const sequential = String(baseSequential + i).padStart(7, '0');
    cnjs.push(generateValidCNJ({
      sequential,
      year: startYear,
