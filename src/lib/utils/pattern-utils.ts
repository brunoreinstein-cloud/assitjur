 ...
 */
export function filterPatternsByRelevance(analysis: AnalysisResult, minConfidence = 50) {
  return {
    ...
    duploPapel: analysis.duploPapel
      .sort((a, b) => {
...
export function getCriticalPatterns(analysis: AnalysisResult) {
  return {
    ...
    highProbHomonyms: analysis.homonimos.filter(h => h.probabilidade === 'ALTA'),
    highConfidenceExchanges: analysis.trocaDireta.filter(t => t.confianca >= 80),
    confirmedTriangulations: analysis.triangulacao.filter(t => t.confianca >= 70)
  }
}
...
export function generatePatternInsights(analysis: AnalysisResult) {
  ...
  if (critical.highRiskDualRole.length > 0) {
    insights.push({
      ...
      recommendation: 'Priorizar contradita baseada em impedimento por interesse',
  ...
  if (critical.professionalWitnesses.length > 0) {
    insights.push({
      ...
      recommendation: 'Investigar padrão antes de contraditar - pode ser testemunha técnica legítima',
      patterns: critical.professionalWitnesses.map(p => `${p.nome} (${p.qtd_depoimentos} depoimentos)`)
    })
  }
  ...
  if (critical.highConfidenceExchanges.length > 0) {
    insights.push({
      ...
      patterns: critical.highConfidenceExchanges.map(t => `${t.testemunhaA} ⇄ ${t.testemunhaB}`)
    })
  }
  ...
  if (critical.highProbHomonyms.length > 0) {
    insights.push({
      ...
      recommendation: 'Verificar CPF/documentos antes de prosseguir com análise',
      patterns: critical.highProbHomonyms.map(h => `${h.nome} (score: ${h.score})`)
    })
  }
...
export function calculateCaseRiskScore(analysis: AnalysisResult): {
  ...
  const highRiskDual = analysis.duploPapel.filter(d => d.risco === 'ALTO').length
  if (highRiskDual > 0) {
  ...
  const highConfTroca = analysis.trocaDireta.filter(t => t.confianca >= 80).length
  if (highConfTroca > 0) {
  ...
  const highProbHomo = analysis.homonimos.filter(h => h.probabilidade === 'ALTA').length
  if (highProbHomo > 0) {
...
export function groupPatternsByType(analysis: AnalysisResult) {
  return [
    ...
    },
    {
      ...
      critical: analysis.homonimos.filter(h => h.probabilidade === 'ALTA').length,
