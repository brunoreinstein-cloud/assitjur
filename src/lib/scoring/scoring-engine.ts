...
export class ScoringEngine {
  ...
  private static readonly WEIGHTS = {
    DUPLO_PAPEL: 0.3, // 30% - Maior peso por ser impedimento legal
    PROVA_EMPRESTADA: 0.25, // 25% - Alto risco de coordenação
    TROCA_DIRETA: 0.2, // 20% - Evidência de reciprocidade
    TRIANGULACAO: 0.15, // 15% - Padrão de rede coordenada
    HOMONIMO: 0.1, // 10% - Menor peso, mais verificação
  };
  ...
  static calculateProcessoScore(
    ...
    const duploPapelScore = this.calculateDuploPapelScore(
      cnj,
      analysis.duploPapel,
    );
    ...
    const provaScore = this.calculateProvaEmprestadaScore(
      cnj,
    ...
    const trocaScore = this.calculateTrocaDiretaScore(
      cnj,
    ...
    const triangScore = this.calculateTriangulacaoScore(
      cnj,
      analysis.triangulacao,
    );
    ...
    const recommendations = this.generateProcessoRecommendations(
      finalScore,
      components,
    );
    const fatoresRisco = this.extractRiskFactors(components);
    const prioridadeContradita = this.calculateContradictaPriority(
      finalScore,
      components,
    );
    ...
    return {
      ...
      classificacao_estrategica: this.getStrategicClassification(
        finalScore,
        components,
      ),
  ...
  static calculateTestemunhaScore(
    ...
    const provaEmprestada = analysis.provaEmprestada.find(
      (p) => p.nome === nome,
    );
    const duploPapel = analysis.duploPapel.find((d) => d.nome === nome);
    const trocas = analysis.trocaDireta.filter(
      (t) => t.testemunhaA === nome || t.testemunhaB === nome,
    );
    const triangulacoes = analysis.triangulacao.filter((tr) =>
      tr.ciclo.includes(nome),
    );
    ...
    if (provaEmprestada) {
      const score =
        this.calculateTestemunhaProvaEmprestadaScore(provaEmprestada);
      components.provaEmprestada = {
        score: score.score,
        weight: 0.35,
    ...
    const finalScore = Math.min(Math.round(totalScore), 100);
    const classification = this.getTestemunhaClassification(
      finalScore,
      provaEmprestada?.qtd_depoimentos || 0,
    );
    const recommendations = this.generateTestemunhaRecommendations(
      finalScore,
      components,
    );
    ...
    return {
      ...
      recomendacao_acao: this.getTestemunhaRecommendation(
        finalScore,
        components,
        alertaProvaEmprestada,
      ),
  ...
  private static calculateDuploPapelScore(
    cnj: string,
    duploPapelResults: DuploPapelResult[],
  ): { score: number; description: string } {
    const relevantCases = duploPapelResults.filter(
      (d) =>
        d.cnjs_como_reclamante.includes(cnj) ||
        d.cnjs_como_testemunha.includes(cnj),
    );
  ...
  private static calculateProvaEmprestadaScore(
    cnj: string,
  ...
  private static calculateTrocaDiretaScore(
    cnj: string,
    trocaResults: TrocaDiretaResult[],
  ): { score: number; description: string } {
    const relevantCases = trocaResults.filter(
      (t) => t.cnjsA.includes(cnj) || t.cnjsB.includes(cnj),
    );
  ...
  private static calculateTriangulacaoScore(
    cnj: string,
    triangResults: TriangulacaoResult[],
  ): { score: number; description: string } {
  ...
  private static calculateHomonimoScore(
    cnj: string,
    homonimoResults: HomonimoResult[],
  ): { score: number; description: string } {
    const relevantCases = homonimoResults.filter((h) =>
      h.cnjs_suspeitos.includes(cnj),
    );
  ...
  private static calculateTestemunhaProvaEmprestadaScore(
    prova: ProvaEmprestadaResult,
  ): { score: number; description: string } {
    let score = 0;
  ...
  private static calculateTestemunhaDuploPapelScore(duplo: DuploPapelResult): {
    score: number;
    description: string;
  } {
  ...
  private static calculateTestemunhaTrocaScore(trocas: TrocaDiretaResult[]): {
    score: number;
    description: string;
  } {
    const avgConfianca =
      trocas.reduce((sum, t) => sum + t.confianca, 0) / trocas.length;
  ...
  private static calculateTestemunhaTriangulacaoScore(
    triangulacoes: TriangulacaoResult[],
  ): { score: number; description: string } {
    const avgConfianca =
      triangulacoes.reduce((sum, t) => sum + t.confianca, 0) /
      triangulacoes.length;
  ...
  private static calculateTestemunhaHomonimoScore(homonimo: HomonimoResult): {
    score: number;
    description: string;
  } {
  ...
  private static getClassification(
    score: number,
  ): "CRITICO" | "ALTO" | "MEDIO" | "BAIXO" | "MINIMO" {
  ...
  private static getTestemunhaClassification(
    score: number,
    qtdDepoimentos: number,
  ): "PROFISSIONAL" | "SUSPEITA" | "NORMAL" | "OCASIONAL" {
  ...
  private static calculateContradictaPriority(
    score: number,
    components: ScoreBreakdown["components"],
  ): "URGENTE" | "ALTA" | "MEDIA" | "BAIXA" | "NAO_RECOMENDADA" {
    // Não recomendar contradita para casos de prova emprestada pura
    if (
      components.provaEmprestada &&
      !components.duploPapel &&
      components.provaEmprestada.score >= 80
    ) {
  ...
  private static generateProcessoRecommendations(
    score: number,
    components: ScoreBreakdown["components"],
  ): string[] {
    ...
    if (components.duploPapel?.score >= 60) {
      recommendations.push(
        "Contraditar baseado em impedimento por interesse (duplo papel)",
      );
    }

    if (components.trocaDireta?.score >= 50) {
      recommendations.push(
        "Explorar reciprocidade para questionar imparcialidade",
      );
    }
    ...
    if (components.provaEmprestada?.score >= 70 && !components.duploPapel) {
      recommendations.push(
        "Investigar se é testemunha técnica antes de contraditar",
      );
    }
  ...
  private static generateTestemunhaRecommendations(
    score: number,
    components: ScoreBreakdown["components"],
  ): string[] {
    const recommendations = [];
    ...
    if (components.trocaDireta?.score >= 40) {
      recommendations.push(
        "Questionar sobre relacionamento com outras testemunhas",
      );
    }
  ...
  private static getTestemunhaRecommendation(
    score: number,
    components: ScoreBreakdown["components"],
    alertaProvaEmprestada: boolean,
  ): string {
  ...
  private static calculateConfidenceLevel(
    ...
    const avgScore =
      Object.values(components).reduce((sum, comp) => sum + comp.score, 0) /
      componentCount;
