...
export const MultiTenantProvider: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => {
  ...
  const loadOrganizations = React.useCallback(async () => {
    ...
    try {
      ...
      // Check cache first
      const cachedOrgs = queryClient.getQueryData<Organization[]>([
        "organizations",
      ...
      if (!current && profile?.organization_id) {
        current =
          orgs.find((org) => org.id === profile.organization_id) || null;
      }
      ...
      if (cachedOrgs && cachedOrgs.length > 0) {
        ...
        toast.warning(
          "Usando dados em cache. Algumas funcionalidades podem estar limitadas.",
        );
      } else {
