...
export async function buildPadroesAgregados(
  ...
  const testemunhasProfissionais = buildTestemunhasProfissionais(
    testemunhas,
    processos,
  );
  ...
  const { concentracaoUF, concentracaoComarca } =
    buildConcentracaoGeografica(processos);
...
function buildTestemunhasProfissionais(
  ...
  return testemunhas
    ...
    .map((testemunha) => {
      ...
      const comarcaMaisFrequente =
        Array.from(comarcaCount.entries()).sort(
          (a, b) => b[1] - a[1],
        )[0]?.[0] || "Desconhecida";
...
function buildAdvogadosRecorrentes(
  ...
  return Array.from(advogadoStats.entries())
    .filter(
      ([_, stats]) => stats.processos.size >= 5 || stats.padroes.size >= 2,
    )
    .map(([nome, stats]) => {
      const ufMaisFrequente =
        Array.from(stats.ufs.entries()).sort((a, b) => b[1] - a[1])[0]?.[0] ||
        "Desconhecida";
...
function buildConcentracaoGeografica(processos: ProcessoData[]): {
  ...
  for (const processo of processos) {
    ...
    padroes.forEach((p) =>
      ufStat.padroes.set(p, (ufStat.padroes.get(p) || 0) + 1),
    );
    ...
    padroes.forEach((p) =>
      comarcaStat.padroes.set(p, (comarcaStat.padroes.get(p) || 0) + 1),
    );
  }
 ...
 */
function buildTendenciaTemporal(
  processos: ProcessoData[],
): TendenciaTemporal[] {
  ...
  const periodos = Array.from(temporalStats.entries()).sort((a, b) =>
 ...
 */
export async function savePadroesAgregados(
  agregados: PadroesAgregados,
): Promise<void> {
