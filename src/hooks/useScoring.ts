...
export function useScoring() {
  ...
  const [processoScores, setProcessoScores] = useState<Map<string, ProcessoScore>>(new Map())
  const [testemunhaScores, setTestemunhaScores] = useState<Map<string, TestemunhaScore>>(new Map())
  ...
  useEffect(() => {
    ...
    Array.from(allTestemunhas).forEach(nome => {
      const score = ScoringEngine.calculateTestemunhaScore(nome, analysisResult)
  ...
  const scoringMetrics = useMemo((): ScoringMetrics | null => {
    ...
    const scores = Array.from(processoScores.values()).map(p => p.score_final)
    const distribuicao = {
    ...
    return {
      ...
      casos_contradita_recomendada: Array.from(processoScores.values())
        .filter(p => ['URGENTE', 'ALTA', 'MEDIA'].includes(p.prioridade_contradita)).length,
      casos_investigacao_necessaria: Array.from(processoScores.values())
        .filter(p => p.prioridade_contradita === 'NAO_RECOMENDADA').length
    }
  }, [processoScores])

  // Get scores by classification
  const getProcessosByClassification = (classification: 'CRITICO' | 'ALTO' | 'MEDIO' | 'BAIXO' | 'MINIMO') => {
    return Array.from(processoScores.values())
      .filter(p => p.score_breakdown.classification === classification)
      .sort((a, b) => b.score_final - a.score_final)
  }

  const getTestemunhasByClassification = (classification: 'PROFISSIONAL' | 'SUSPEITA' | 'NORMAL' | 'OCASIONAL') => {
  ...
  const getHighPriorityCases = () => {
    return Array.from(processoScores.values())
      ...
      .sort((a, b) => {
        ...
        return priorityOrder[b.prioridade_contradita as keyof typeof priorityOrder] - 
  ...
  return {
    ...
    urgentCases: getHighPriorityCases().filter(p => p.prioridade_contradita === 'URGENTE').length
  }
}
...
export function useScoreTracker(identifier: string, type: 'processo' | 'testemunha') {
    ...
    if (currentScore && !history) {
      setHistory({
        ...
        score_changes: [{
          ...
          changed_components: Object.keys(currentScore.score_breakdown.components)
        }],
        trend: 'STABLE'
      })
    }
  ...
  return {
    ...
    scoreChange: history && history.score_changes.length > 1
...
export function useScoreMonitoring() {
  const { processoScores, testemunhaScores, criticalCases, urgentCases } = useScoring()
