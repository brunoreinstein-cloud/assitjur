...
import {
  filterPatternsByRelevance,
  getCriticalPatterns,
  generatePatternInsights,
  calculateCaseRiskScore,
} from "@/lib/utils/pattern-utils";
...
export function usePatternAnalysis() {
  ...
  const analysisMutation = useMutation({
    mutationFn: (request: AnalysisRequest) =>
      PatternAnalysisService.runAnalysis(request),
    onSuccess: (data) => {
  ...
  const caseAnalysisMutation = useMutation({
    mutationFn: (cnjs: string[]) =>
      PatternAnalysisService.analyzeSpecificCases(cnjs),
    onSuccess: (data) => setLastAnalysis(data),
  });
  ...
  const summary = currentAnalysis
    ? PatternAnalysisService.extractSummary(currentAnalysis)
    : null;

  const filteredPatterns = currentAnalysis
    ? filterPatternsByRelevance(currentAnalysis)
    : null;

  const criticalPatterns = currentAnalysis
    ? getCriticalPatterns(currentAnalysis)
    : null;

  const insights = currentAnalysis
    ? generatePatternInsights(currentAnalysis)
    : [];

  const riskAssessment = currentAnalysis
    ? calculateCaseRiskScore(currentAnalysis)
    : null;
  ...
  const formatForReport = (analysis?: AnalysisResult) => {
    ...
    return dataToFormat
      ? PatternAnalysisService.formatForReport(dataToFormat)
      : null;
  };
  ...
  return {
    ...
    hasCriticalIssues: criticalPatterns
      ? Object.values(criticalPatterns).some((arr) => arr.length > 0)
      : false,
 ...
 */
export function useSpecificPatternAnalysis(
  patternType:
    | "trocaDireta"
    | "triangulacao"
    | "duploPapel"
...
export function usePatternMonitoring(intervalMs = 30000) {
  ...
  const [previousCriticalCount, setPreviousCriticalCount] = useState<
    number | null
  >(null);
