...
import { withErrorHandling, apiCall, isValidOrgId, createError } from '@/lib/error-handling';
...
export function useAssistJurStats() {
  ...
  const { data: stats, isLoading: statsLoading } = useQuery({
    ...
    queryFn: async () => {
      if (!isValidOrgId(profile?.organization_id)) {
        throw createError.authentication('Organização não encontrada para estatísticas');
      }
