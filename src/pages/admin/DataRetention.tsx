...
export default function DataRetention() {
  ...
  const handleExecuteCleanup = async (policyId: string) => {
    ...
    try {
      ...
      toast.success(`Limpeza executada: ${data.result.records_affected} registros afetados`);
      fetchData();
    } catch (error) {
  ...
  const handleBatchCleanup = async () => {
    ...
    try {
      ...
      toast.success(`Limpeza em lote executada: ${data.processed} políticas processadas`);
      fetchData();
    } catch (error) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'COMPLETED':
        return <Badge className="bg-success"><CheckCircle className="h-3 w-3 mr-1" />Concluído</Badge>;
      case 'FAILED':
        return <Badge variant="destructive"><AlertTriangle className="h-3 w-3 mr-1" />Falhou</Badge>;
      case 'STARTED':
        return <Badge variant="secondary"><Clock className="h-3 w-3 mr-1" />Iniciado</Badge>;
      default:
  ...
  return (
    <div className="space-y-6">
      ...
      <Alert>
        ...
        <AlertDescription>
          <strong>Compliance LGPD:</strong> As políticas de retenção automatizada garantem o descarte 
          seguro de dados após o prazo legal necessário, conforme Art. 6º, VII da LGPD.
        </AlertDescription>
      </Alert>
              ...
              <Card key={policy.id}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-4">
                      <div className="flex items-center gap-2">
                        ...
                        <Switch
                          ...
                          onCheckedChange={(checked) => 
                            handleUpdatePolicy(policy.id, { auto_cleanup: checked })
                          }
                        />
                      </div>
                      <Button
                      ...
                      >
                        ...
                        {executing === policy.id ? 'Executando...' : 'Executar Limpeza'}
                      </Button>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                      <label className="text-sm font-medium">Retenção (meses):</label>
                      <Input
                        type="number"
                    ...
                    <div>
                      <label className="text-sm font-medium">Última Limpeza:</label>
                      <p className="text-sm text-muted-foreground mt-1">
                        {policy.last_cleanup_at 
                          ? new Date(policy.last_cleanup_at).toLocaleString('pt-BR')
                          : 'Nunca executada'
                        }
                      </p>
                    </div>
                    <div>
                      <label className="text-sm font-medium">Próxima Limpeza:</label>
                      <p className="text-sm text-muted-foreground mt-1">
                        {policy.next_cleanup_at 
                          ? new Date(policy.next_cleanup_at).toLocaleString('pt-BR')
                          : 'Não agendada'
                        }
                      </p>
                    </div>
                  ...
                  logs.map((log) => (
                    <div key={log.id} className="flex items-center justify-between p-4 border rounded-lg">
                      <div>
                        ...
                        {log.error_message && (
                          <p className="text-sm text-destructive">Erro: {log.error_message}</p>
                        )}
                      </div>
                      <div className="text-right text-sm text-muted-foreground">
                        <div>Iniciado: {new Date(log.started_at).toLocaleString('pt-BR')}</div>
                        {log.completed_at && (
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Políticas com auto-limpeza habilitada são executadas mensalmente
                  </p>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-sm text-muted-foreground">
                    Dados críticos (processos ativos) são protegidos contra exclusão acidental.
                    Logs de auditoria são mantidos para compliance regulatório.
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <Alert>
                  ...
                  <AlertDescription>
                    <strong>Base Legal LGPD:</strong> Art. 6º, VII - "não conservação dos dados pessoais 
                    por tempo superior ao necessário para o alcance das finalidades para as quais foram coletados"
                  </AlertDescription>
                </Alert>
                ...
                <div className="space-y-2">
                  ...
                  <ul className="text-sm text-muted-foreground space-y-1">
                    <li>• Processos: Mantidos enquanto tramitam + 5 anos (prescrição)</li>
                    <li>• Logs de Auditoria: 2 anos para investigações</li>
                    <li>• Dados de IA: 1 ano para melhorias do serviço</li>
                    <li>• Solicitações LGPD: 3 anos para evidência de compliance</li>
                  </ul>
                </div>
              </CardContent>
            </Card>
