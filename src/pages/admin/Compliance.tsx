...
import { Shield, Eye, CheckCircle, Clock, XCircle, Download, Users, FileText, AlertTriangle } from 'lucide-react';
...
export default function Compliance() {
  ...
  const getStatusBadge = (status: string) => {
    const statusConfig = {
      ...
      PROCESSING: { variant: 'default' as const, icon: Eye, text: 'Processando' },
      COMPLETED: { variant: 'default' as const, icon: CheckCircle, text: 'Concluída' },
      REJECTED: { variant: 'destructive' as const, icon: XCircle, text: 'Rejeitada' }
    };

    const config = statusConfig[status as keyof typeof statusConfig] || statusConfig.PENDING;
    const Icon = config.icon;

    const extraClass = status === 'COMPLETED' ? 'bg-success text-success-foreground' : '';
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-muted-foreground">
          Gestão de solicitações e logs de acesso conforme a Lei Geral de Proteção de Dados
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-sm font-medium text-muted-foreground">Logs de Acesso</p>
      ...
      <Tabs defaultValue="requests" className="space-y-4">
            ...
            {filteredRequests.length === 0 ? (
              <Card>
                <CardContent className="p-8 text-center">
                  ...
                  <p className="text-muted-foreground">Nenhuma solicitação LGPD encontrada</p>
                </CardContent>
              </Card>
            ) : (
                ...
                <Card key={request.id}>
                  ...
                  <CardContent>
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      <div>
                        <strong>Criada em:</strong> {new Date(request.created_at).toLocaleString('pt-BR')}
                      </div>
                      <div>
                        <strong>Expira em:</strong> {new Date(request.expires_at).toLocaleString('pt-BR')}
                      </div>
                      {request.completed_at && (
                        <div>
                          <strong>Concluída em:</strong> {new Date(request.completed_at).toLocaleString('pt-BR')}
                        </div>
                      )}
                    </div>
                  </CardContent>
                ...
                {accessLogs.slice(0, 20).map((log) => (
                  <div key={log.id} className="flex items-center justify-between py-2 border-b">
                    <div>
                      ...
                      <span className="text-sm text-muted-foreground">{log.access_type}</span>
                    </div>
        ...
        <TabsContent value="settings" className="space-y-4">
          <Alert>
            ...
            <AlertDescription>
              <strong>Portal do Titular:</strong> O portal público para solicitações LGPD está disponível em{' '}
              <a href="/portal-titular" className="text-primary underline" target="_blank">
                /portal-titular
              </a>
            </AlertDescription>
          </Alert>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Dados são retidos por 24 meses após o fim da necessidade processual, 
                  conforme política de retenção configurada.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  CPFs, nomes e dados sensíveis são mascarados automaticamente em 
                  exports e relatórios por padrão.
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground">
                  Art. 7º, VI da LGPD - Execução regular de direitos em processo judicial
                </p>
              </div>
            </CardContent>
          </Card>
